...
CREATE FUNCTION [dbo].[cbrDataReported2] ( @AccountId INT)
RETURNS TABLE
AS  
    RETURN 
		with xcepts as (	select d.debtorid, ROW_NUMBER() OVER (PARTITION BY x.debtorid ORDER BY case when d.seq = 0 then 'Acctx' Else 'Dbtrx' end ASC) AS topError,
								...
								from CbrDataExceptionDtlex(null) x inner join cbrDatafx2(null) d on d.number = x.number and  d.debtorid = x.debtorid
		...
		left outer join tempcbrDebtorHistory h2 on  h.recordID = h2.recordID and h2.debtorID = d.DebtorID 
