...
SET ANSI_NULLS ON
GO
CREATE FUNCTION [dbo].[CbrDataExceptionDtl](@Accountid int)
returns table as
return
select      x.number,
			x.debtorid,
...
from		dbo.cbr_exceptions x inner join debtors d on d.debtorid = x.debtorid where x.Number = @Accountid or @Accountid is null;       
GO
