...
SET ANSI_NULLS ON
GO
CREATE FUNCTION [dbo].[CbrDataExceptionDtlex](@Accountid int)
returns table as
return
...
*/
	...
	with exceptions as (
			...
			union all
			select  Number, DebtorId, Responsible, CbrException, CbrExclude, OutOfStatute, DebtorExceptions, IsBusiness, RptDtException, MinBalException  FROM dbo.cbr_exceptions where @Accountid is null
			),
	details as (
