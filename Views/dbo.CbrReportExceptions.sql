...
SET ANSI_NULLS ON
GO
CREATE VIEW [dbo].[CbrReportExceptions]
as
select      x.StatusCbrReport, x.StatusCbrDelete , x.reportable,
			Case when x.cbrenabled = 1 then 'cbrenabled' else '' end as cbrenabled,
			Case when x.cbroverride = 1 then 'cbrOverride' else '' end as cbrOverride,
...
group by    x.StatusCbrReport, x.StatusCbrDelete , x.reportable,x.CbrException,x.debtorexceptions,
			Case when x.cbrenabled = 1 then 'cbrenabled' else '' end ,
			Case when x.cbroverride = 1 then 'cbrOverride' else '' end,
			case when x.cbrexclude = 1 then 'DebtorExcluded' else '' end  ,
            case when x.responsible = 0 and d.isauthorizedaccountuser = 0 then 'DebtorNotResponsible' else '' end ,
            case when x.responsible = 1 and d.isauthorizedaccountuser = 1 then 'AuthorizedUserIsResponsible' else '' end ,

            case when abs(x.cbrexception) & 1 = 1 then 'NullDelinquencyDt' else '' end ,
            case when abs(x.cbrexception) & 2 = 2 then 'DelinquencyDt>Received' else '' end ,
			case when abs(x.cbrexception) & 4 = 4 then 'DelinquencyDtWithin30daysOfReceived' else '' end ,
			case when abs(x.cbrexception) & 8 = 8 then 'InvalidAcctType' else '' end ,
			case when abs(x.cbrexception) & 16 = 16 then 'InvalidContractDate' else '' end ,
			case when abs(x.cbrexception) & 32 = 32 then 'InvalidConsumerAccountNumber' else '' end ,
			case when abs(x.cbrexception) & 64 = 64 then 'MissingChargeOffRecord' else '' end ,
			case when abs(x.cbrexception) & 128 = 128 then 'MissingChargeOffAmount' else '' end ,
			case when abs(x.cbrexception) & 256 = 256 then 'MissingSecondaryAgencyIdenitifier' else '' end ,
			case when abs(x.cbrexception) & 512 = 512 then 'MissingSecondaryAccountNumber' else '' end ,
			case when abs(x.cbrexception) & 1024 = 1024 then 'OpenChargeOffNotInBankruptcy' else '' end ,
			case when abs(x.cbrexception) & 2048 = 2048 then 'MissingAcctOriginalCreditor' else '' end ,
			case when abs(x.cbrexception) & 4096 = 4096 then 'MissingDefaultOriginalCreditor' else '' end ,
			case when abs(x.cbrexception) & 8192 = 8192 then 'MissingCreditorClass' else '' end ,
			case when abs(x.cbrexception) & 16384 = 16384 then 'MedicalWithin365Days' else '' end ,
			case when abs(x.CbrException) & 32768 = 32768 then 'InvalidContractToPay' else '' end ,
			case when abs(x.CbrException) & 2097152 = 2097152 then 'InvalidOriginalLoanAmount' else '' end ,
                 
            case when x.OutofStatute = 1 then 'OutofStatute' else '' end ,

            case when abs(x.DebtorExceptions) & 65536 = 65536 then 'NullDebtorName' else '' end , 
            case when abs(x.DebtorExceptions) & 131072 = 131072 then 'InvalidDebtorName' else '' end , 
            case when abs(x.DebtorExceptions) & 262144 = 262144 then 'NullDebtorZipcode' else '' end , 
            case when abs(x.DebtorExceptions) & 524288 = 524288 then 'InvalidDebtorZipcode' else '' end , 
            case when abs(x.DebtorExceptions) & 1048576 = 1048576 then 'ZeroedDebtorZipcode' else '' end , 
            case when abs(x.DebtorExceptions) & 4194304 = 4194304 then 'InvalidDOB' else '' end, 
			case when abs(x.DebtorExceptions) & 8388608 = 8388608 then 'MissingSSN&DOB' else '' end , 
			case when abs(x.DebtorExceptions) & 16777216 = 16777216 then 'InvalidState' else '' end ,
			case when abs(x.DebtorExceptions) & 33554432 = 33554432 then 'MissingStreetAddress' else '' end ,
			case when abs(x.DebtorExceptions) & 67108864 = 67108864 then 'MissingCity' else '' end ,
			case when abs(x.DebtorExceptions) & 134217728 = 134217728 then 'InvalidSSN' else '' end ,
			
			case when x.IsBusiness = 1 then 'Business' else '' end,
            case when x.RptDtException = 1 then 'ReportDateNotMet' else '' end,
            case when x.MinBalException = 1 then 'MinBalException' else '' end
