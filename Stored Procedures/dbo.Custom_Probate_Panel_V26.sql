...
-- =============================================
CREATE PROCEDURE [dbo].[Custom_Probate_Panel_V26] 
	-- Add the parameters for the stored procedure here
	@number int,
  @userid   int

--exec custom_probate_panel_V26 13816133             , 268
...
select cp.ID, Case when [cmty prop] = 'CP' then 'YES' else 'NO' end as [CMTY PROP], 
	...
	CASE WHEN m.customer IN ('0001121', '0001134', '0001188', '0001549') THEN m.PreviousCreditor + ' successor in interest to ' + m.OriginalCreditor 
	...
	CASE WHEN m.customer IN ('0001219', '0001220') THEN c.name 
		...
		WHEN m.customer IN ('0001749') AND m.originalcreditor = 'ELAN' THEN 'U.S. Bank National Association dba Elan Financial Services' 
		WHEN m.customer IN ('0001749') AND m.originalcreditor = 'ACG Card Services' THEN 'US Bank National Association dba ACG Card Services'
		ELSE m.OriginalCreditor END AS OriginalCreditor, CASE WHEN m.customer IN ('0001220') THEN '' WHEN m.customer IN ('0001749') THEN 'Unsecured' ELSE m.PreviousCreditor END AS PreviousCreditor, m.account, CONVERT(VARCHAR(10), m.ContractDate, 101) AS [ContractDate], '$' + CONVERT(VARCHAR(16), m.current0, 1) AS [current],
	...
	SELECT CASE WHEN id in (2219) THEN RoleID WHEN roleid = 16 THEN 2 ELSE RoleID END AS RoleID
