...
-- Description:	Exports maintenance records sent in a placement file
-- Changes:		09/14/2023 BGM Added OpRequest records to be added into the maintnance file for recalls.
-- =============================================
CREATE PROCEDURE [dbo].[Custom_Equabli_Export_Placement_File_Maint_Records_V34_BU]
...
WHERE ceci.account_id NOT IN (SELECT account_id FROM Custom_Equabli_Account_Info)
...
WHERE ceai.is_primary = 'N' AND ceai.account_id NOT IN (SELECT account_id FROM Custom_Equabli_Account_Info)
...
WHERE ceei.is_primary = 'N' AND ceei.account_id NOT IN (SELECT account_id FROM Custom_Equabli_Account_Info)
...
WHERE ceepi.account_id NOT IN (SELECT account_id FROM Custom_Equabli_Account_Info)
...
WHERE cedpi.account_id NOT IN (SELECT account_id FROM Custom_Equabli_Account_Info)
...
WHERE cesi.account_id NOT IN (SELECT account_id FROM Custom_Equabli_Account_Info)
...
WHERE ceshi.account_id NOT IN (SELECT account_id FROM Custom_Equabli_Account_Info)
...
WHERE cersi.account_id NOT IN (SELECT account_id FROM Custom_Equabli_Account_Info)
...
WHERE ceasci.account_id NOT IN (SELECT account_id FROM Custom_Equabli_Account_Info) AND ceasci.account_id NOT IN (SELECT account_id FROM Custom_Equabli_OpRequest_Info ceori WITH (NOLOCK))
...
WHERE ceaaci.account_id NOT IN (SELECT account_id FROM Custom_Equabli_Account_Info)
...
WHERE ceai.account_id NOT IN (SELECT account_id FROM Custom_Equabli_Account_Info)

--OpRequests
SELECT ceori.record_type, ceori.account_id, ceori.client_account_number, ceori.client_id, ceori.client_consumer_number, ceori.consumer_id, ceori.request_number
	 , ceori.operation_requesttype, ceori.queuereason, ceori.description, ceori.request_source, ceori.request_source_id, ceori.dt_request, ceori.dt_fulfillment, ceori.request_status 
FROM Custom_Equabli_OpRequest_Info ceori WITH (NOLOCK) 
WHERE ceori.account_id NOT IN (SELECT account_id FROM Custom_Equabli_Account_Info)
END
GO
