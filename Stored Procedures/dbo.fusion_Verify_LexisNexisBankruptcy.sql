...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[fusion_Verify_LexisNexisBankruptcy]
AS
BEGIN
--make sure that it created the proc
if not exists(select * from INFORMATION_SCHEMA.ROUTINES where ROUTINE_NAME='fusion_Create_LexisNexisBankruptcy')
...
if not exists(select * from Services where Description='LexisNexis Bankruptcy')
	RAISERROR (N'Service missing',15,1,N'number',5);

Declare @expectedManifestId uniqueidentifier
set @expectedManifestId='366A78FA-62A6-410D-ABEE-A3E71E165073'
...
if not exists(select * from Custom_CustomerReference where TreePath='Clients\Latitude Fusion\LexisNexis\Bankruptcy')
