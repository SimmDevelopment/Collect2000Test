...
-- =============================================
CREATE PROCEDURE [dbo].[Custom_JHCapital_StatusUpdate_DN2]
...
BEGIN
	...
	WHEN status ='ACT' THEN '112000' WHEN status ='NON' THEN '191002' WHEN status IN ('PPA', 'PDC', 'PCC') THEN '113020' WHEN status = 'MIL' THEN '910208'
...
WHERE customer IN (SELECT customerid FROM fact f WITH (NOLOCK) WHERE CustomGroupID = 186) AND returned IS NULL and id2 not in ('AllGate','ARS-JMET')
