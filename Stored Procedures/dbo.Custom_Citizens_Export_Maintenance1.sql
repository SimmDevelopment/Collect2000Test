...
--exec Custom_Citizens_Export_Maintenance1 '20190416', '20190416', '0002111|0002112'

CREATE      PROCEDURE [dbo].[Custom_Citizens_Export_Maintenance1]
@BeginDate datetime,
@EndDate datetime,
...
BEGIN
        ...
        DECLARE @dps VARCHAR(50)
        DECLARE @mio VARCHAR(20)
        DECLARE cur CURSOR FOR
        
        --load the cursor with the DPS ID and MIO Code from the customer table
        SELECT distinct  c.customtext1, id2
		FROM master m WITH (NOLOCK) INNER JOIN customer c WITH (NOLOCK) ON m.Customer = c.customer
		WHERE   c.customer IN (SELECT  string FROM dbo.CustomStringToSet(@customer, '|'))
	
        OPEN cur

		--get the information from the cursor and into the variables
        FETCH FROM cur INTO @dps, @mio
...
CREATE TABLE #CitizensReturns(number int)
	...
	c.customtext1 = @dps AND m.id2 = @mio
	...
	ON s.code = m.status
	WHERE 	c.customtext1 = @dps AND m.id2 = @mio 
	AND m.status IN('AEX','ATY','B07','B11','B13','BKY','FRD','PIF','SIF','RCL', 'RFP', 'STL', 'DEC', 'CCR', 'CND', 'CAD', 'DIP') AND 
...
*/
	...
	CASE WHEN @dps = '3801' 
		...
		WHEN @dps = '3802'
		THEN CASE
			WHEN m.STATUS = 'AEX' THEN '262'
			WHEN m.STATUS = 'B07' THEN '264'
			WHEN m.STATUS = 'B11' THEN '264'
			WHEN m.STATUS = 'B13' THEN '264'
			WHEN m.STATUS = 'BKY' THEN '264'
			WHEN m.STATUS = 'DEC' THEN '265'
			WHEN m.STATUS = 'CND' THEN '26C'
			WHEN m.STATUS = 'CAD' THEN '26C'
			WHEN m.STATUS = 'MIL' THEN '26S'
			WHEN m.STATUS = 'OOS' THEN '16L'
			END 		
		WHEN @dps = '3803'
		THEN CASE
...
INSERT INTO #Citizens_TempUploadMaintenance
	...
	WHERE (m.number IN(SELECT number FROM #CitizensReturns)) and
          (m.Status in ('PIF', 'SIF')) AND DATEADD(dd, 10, m.closed) BETWEEN @beginDate AND @endDate
	...
	Loan_Code)
			...
			case 				
				...
				when [action] IN ('TR', 'TO', 'TE', 'TI', 'TC', 'DIAL', 'IC') and result IN ('pp', 'pay') then '95'
				...
				when [action] IN ('IC', 'DT') AND result NOT IN ('pp', 'pay') then '97'
				else '90'				
				 end,        	 
			'01    ',
			isnull(substring(replace(replace(REPLACE(replace(convert(varchar(40),n.comment),char(13) + char(10),' '), char(13), ' '), char(10), ' '), '	', ' '), 0, 40), ''),
			'X',
	c.customtext1 AS DPS_ID,
	c.customtext1 AS Recoverer_ID,
	m.id2 AS Loan_Code
	...
	where dbo.date(n.created) between @BeginDate and @EndDate and convert(varchar(100), n.comment) <> '' and
	      c.customtext1 = @dps AND m.id2 = @mio
	      and [action] in ('TR', 'TO', 'TE', 'DT', 'TI', 'IC', 'TC', 'DIAL', '+++++', 'co')
...
SELECT 
			...
			'02    ',
			isnull(substring(replace(replace(replace(replace(convert(varchar(100),n.comment),char(13) + char(10),' '), char(13), ' '), char(10), ' '), '	', ' '), 40, 40), ''),
		'X',
		c.customtext1 AS DPS_ID,
	c.customtext1 AS Recoverer_ID,
	m.id2 AS Loan_Code
	...
	INNER JOIN customer c WITH(NOLOCK)
	on c.customer = m.customer
	where dbo.date(n.created) between @BeginDate and @EndDate and isnull(substring(replace(replace(replace(replace(convert(varchar(100),n.comment),char(13) + char(10),' '), char(13), ' '), char(10), ' '), '	', ' '), 40, 40), '') <> '' and
	      c.customtext1 = @dps AND m.id2 = @mio and [action] in ('TR', 'TO', 'TE', 'DT', 'TI', 'IC', 'TC', 'DIAL', '+++++', 'co')
...
SELECT 
			...
			'03    ',
			isnull(substring(replace(replace(replace(replace(convert(varchar(1000),n.comment),char(13) + char(10),' '), char(13), ' '), char(10), ' '), '	', ' '), 80, 40), ''),
			'X',
	c.customtext1 AS DPS_ID,
	c.customtext1 AS Recoverer_ID,
	m.id2 AS Loan_Code
	...
	INNER JOIN customer c WITH(NOLOCK)
	on c.customer = m.customer
	where dbo.date(n.created) between @BeginDate and @EndDate and isnull(substring(replace(replace(replace(replace(convert(varchar(1000),n.comment),char(13) + char(10),' '), char(13), ' '), char(10), ' '), '	', ' '), 80, 40), '') <> '' and
	      c.customtext1 = @dps AND m.id2 = @mio and [action] in ('TR', 'TO', 'TE', 'DT', 'TI', 'IC', 'TC', 'DIAL', '+++++', 'co')
...
SELECT 
			...
			'04    ',
			isnull(substring(replace(replace(replace(replace(convert(varchar(1000),n.comment),char(13) + char(10),' '), char(13), ' '), char(10), ' '), '	', ' '), 120, 40), ''),
			'X',
	c.customtext1 AS DPS_ID,
	c.customtext1 AS Recoverer_ID,
	m.id2 AS Loan_Code
	...
	INNER JOIN customer c WITH(NOLOCK)
	on c.customer = m.customer
	where dbo.date(n.created) between @BeginDate and @EndDate and isnull(substring(replace(replace(replace(replace(convert(varchar(1000),n.comment),char(13) + char(10),' '), char(13), ' '), char(10), ' '), '	', ' '), 120, 40), '') <> '' and
	      c.customtext1 = @dps AND m.id2 = @mio and [action] in ('TR', 'TO', 'TE', 'DT', 'TI', 'IC', 'TC', 'DIAL', '+++++', 'co')
...
SELECT 
			...
			'05    ',
			isnull(substring(replace(replace(replace(replace(convert(varchar(1000),n.comment),char(13) + char(10),' '), char(13), ' '), char(10), ' '), '	', ' '), 160, 40), ''),
			'X',
	c.customtext1 AS DPS_ID,
	c.customtext1 AS Recoverer_ID,
	m.id2 AS Loan_Code
	...
	INNER JOIN customer c WITH(NOLOCK)
	on c.customer = m.customer
	where dbo.date(n.created) between @BeginDate and @EndDate and isnull(substring(replace(replace(replace(replace(convert(varchar(1000),n.comment),char(13) + char(10),' '), char(13), ' '), char(10), ' '), '	', ' '), 160, 40), '') <> '' and
	      c.customtext1 = @dps AND m.id2 = @mio and [action] in ('TR', 'TO', 'TE', 'DT', 'TI', 'IC', 'TC', 'DIAL', '+++++', 'co')
...
SELECT 
			...
			'06    ',
			isnull(substring(replace(replace(replace(replace(convert(varchar(1000),n.comment),char(13) + char(10),' '), char(13), ' '), char(10), ' '), '	', ' '), 200, 40), ''),
			'X',
	c.customtext1 AS DPS_ID,
	c.customtext1 AS Recoverer_ID,
	m.id2 AS Loan_Code
	...
	INNER JOIN customer c WITH(NOLOCK)
	on c.customer = m.customer
	where dbo.date(n.created) between @BeginDate and @EndDate and isnull(substring(replace(replace(replace(replace(convert(varchar(1000),n.comment),char(13) + char(10),' '), char(13), ' '), char(10), ' '), '	', ' '), 200, 40), '') <> '' and
	      c.customtext1 = @dps AND m.id2 = @mio and [action] in ('TR', 'TO', 'TE', 'DT', 'TI', 'IC', 'TC', 'DIAL', '+++++', 'co')
...
SELECT 
			...
			'07    ',
			isnull(substring(replace(replace(replace(replace(convert(varchar(1000),n.comment),char(13) + char(10),' '), char(13), ' '), char(10), ' '), '	', ' '), 240, 40), ''),
			'X',
	c.customtext1 AS DPS_ID,
	c.customtext1 AS Recoverer_ID,
	m.id2 AS Loan_Code
	...
	INNER JOIN customer c WITH(NOLOCK)
	on c.customer = m.customer
	where dbo.date(n.created) between @BeginDate and @EndDate and isnull(substring(replace(replace(replace(replace(convert(varchar(1000),n.comment),char(13) + char(10),' '), char(13), ' '), char(10), ' '), '	', ' '), 240, 40), '') <> '' and
	      c.customtext1 = @dps AND m.id2 = @mio and [action] in ('TR', 'TO', 'TE', 'DT', 'TI', 'IC', 'TC', 'DIAL', '+++++', 'co')
...
SELECT 
			...
			'08    ',
			isnull(substring(replace(replace(replace(replace(convert(varchar(1000),n.comment),char(13) + char(10),' '), char(13), ' '), char(10), ' '), '	', ' '), 280, 40), ''),
			'X',
	c.customtext1 AS DPS_ID,
	c.customtext1 AS Recoverer_ID,
	m.id2 AS Loan_Code
	...
	INNER JOIN customer c WITH(NOLOCK)
	on c.customer = m.customer
	where dbo.date(n.created) between @BeginDate and @EndDate and isnull(substring(replace(replace(replace(replace(convert(varchar(1000),n.comment),char(13) + char(10),' '), char(13), ' '), char(10), ' '), '	', ' '), 280, 40), '') <> '' and
	      c.customtext1 = @dps AND m.id2 = @mio and [action] in ('TR', 'TO', 'TE', 'DT', 'TI', 'IC', 'TC', 'DIAL', '+++++', 'co')
UNION
...
SELECT 
			...
			'09    ',
			isnull(substring(replace(replace(replace(replace(convert(varchar(1000),n.comment),char(13) + char(10),' '), char(13), ' '), char(10), ' '), '	', ' '), 320, 40), ''),
			'X',
	c.customtext1 AS DPS_ID,
	c.customtext1 AS Recoverer_ID,
	m.id2 AS Loan_Code
	...
	INNER JOIN customer c WITH(NOLOCK)
	on c.customer = m.customer
	where dbo.date(n.created) between @BeginDate and @EndDate and isnull(substring(replace(replace(replace(replace(convert(varchar(1000),n.comment),char(13) + char(10),' '), char(13), ' '), char(10), ' '), '	', ' '), 320, 40), '') <> '' and
	      c.customtext1 = @dps AND m.id2 = @mio and [action] in ('TR', 'TO', 'TE', 'DT', 'TI', 'IC', 'TC', 'DIAL', '+++++', 'co')
...
SELECT 
			...
			'10    ',
			isnull(substring(replace(replace(replace(replace(convert(varchar(1000),n.comment),char(13) + char(10),' '), char(13), ' '), char(10), ' '), '	', ' '), 360, 40), ''),
			'X',
	c.customtext1 AS DPS_ID,
	c.customtext1 AS Recoverer_ID,
	m.id2 AS Loan_Code
	...
	INNER JOIN customer c WITH(NOLOCK)
	on c.customer = m.customer
	where dbo.date(n.created) between @BeginDate and @EndDate and isnull(substring(replace(replace(replace(replace(convert(varchar(1000),n.comment),char(13) + char(10),' '), char(13), ' '), char(10), ' '), '	', ' '), 360, 40), '') <> '' and
	      c.customtext1 = @dps AND m.id2 = @mio and [action] in ('TR', 'TO', 'TE', 'DT', 'TI', 'IC', 'TC', 'DIAL', '+++++', 'co')
...
--	      c.customtext1 = @dps AND m.id2 = @mio and [action] not in ('TR', 'TO', 'TE', 'DT', 'TI', 'desk', 'mnual', 'phone', 'addr', '+++++', 'prom', 'dial') and user0 not in ('exg', 'exchng', 'exchange')
...
--			'90',        	 
--			'02    ',
--			isnull(substring(replace(replace(replace(replace(convert(varchar(1000),n.comment),char(13) + char(10),' '), char(13), ' '), char(10), ' '), '	', ' '), 40, 40), ''),
--			'X',
--	c.customtext1 AS DPS_ID,
--	c.customtext1 AS Recoverer_ID,
--	m.id2 AS Loan_Code
...
--	INNER JOIN customer c WITH(NOLOCK)
--	on c.customer = m.customer
--	where dbo.date(n.created) between @BeginDate and @EndDate and isnull(substring(replace(replace(replace(replace(convert(varchar(1000),n.comment),char(13) + char(10),' '), char(13), ' '), char(10), ' '), '	', ' '), 40, 40), '') <> '' and
--	      c.customtext1 = @dps AND m.id2 = @mio and [action] not in ('TR', 'TO', 'TE', 'DT', 'TI', 'desk', 'mnual', 'phone', 'addr', '+++++', 'prom', 'dial') and user0 not in ('exg', 'exchng', 'exchange')
...
--			and len(convert(varchar(1000), n.comment)) > 360
	...
	-- Return all records...
	SELECT *, REPLACE(CONVERT(char, TransDate, 111), '/', '') as BatchDate, 
	CASE WHEN recoverer_code = '3803' THEN '3274' ELSE '3264' END AS Bank_ID FROM #Citizens_TempUploadMaintenance
	WHERE NewValue IS NOT NULL AND RTRIM(LTRIM(NewValue)) != '' AND @@ROWCOUNT > 0
