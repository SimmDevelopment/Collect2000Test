...
-- =============================================
CREATE PROCEDURE [dbo].[Custom_Citizens_FP_Export_Placements] 
...
FROM Custom_Citizens_First_Party_Master_Temp_File c WITH (NOLOCK)
WHERE acct_num NOT IN (SELECT account FROM master WITH (NOLOCK) WHERE customer = '0002226' AND (closed IS NULL or status in ('dec', 'bky', 'dsp', 'vrb', 'vrd', 'rsk', 'mil')))
AND c_flag IN ('Y', '1')
