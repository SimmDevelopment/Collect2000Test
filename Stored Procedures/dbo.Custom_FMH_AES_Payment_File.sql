...
-- =============================================
CREATE PROCEDURE [dbo].[Custom_FMH_AES_Payment_File]
...
where customer in (select customerid from fact with (nolock) where customgroupid = 134) and invoiced = dbo.date(getdate()) and batchtype = 'pu'
...
where p.customer in (select customerid from fact with (nolock) where customgroupid = 134) and invoiced = dbo.date(getdate()) and batchtype = 'pu'
...
select 'TRAILER' as recordtype,
	...
	replace(sum(case when batchtype like '%r' then -(p.paid1 + p.paid2) else (p.paid1 + p.paid2) end), '.', '') as totalpayments,
	'' as filler,
	'PA' as regionind
from payhistory p with (nolock) inner join master m with (nolock) on p.number = m.number
where p.customer in (select customerid from fact with (nolock) where customgroupid = 134) and invoiced = dbo.date(getdate()) and batchtype = 'pu'
