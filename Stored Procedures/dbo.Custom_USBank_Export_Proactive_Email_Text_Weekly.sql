...
--01/08/2024 BGM Added code for total dollars collected within 5 days of email and changed group code to 389
-- =============================================
CREATE PROCEDURE [dbo].[Custom_USBank_Export_Proactive_Email_Text_Weekly]
...
, FORMAT(dbo.F_END_OF_MONTH(DATEADD(dd, -7, GETDATE())), 'M/dd/yy') AS [EOM Date]
--, (SELECT COUNT(*)
...
--AND CAST(n.created AS DATE) BETWEEN CAST(dbo.F_START_OF_MONTH(DATEADD(dd, -7, GETDATE())) AS DATE) AND CAST(dbo.F_END_OF_MONTH(DATEADD(dd, -7, GETDATE())) AS DATE)
...
, (SELECT COUNT(p.totalpaid)
FROM master m WITH (NOLOCK) INNER JOIN notes n WITH (NOLOCK) ON m.number = n.number
INNER JOIN payhistory p WITH (NOLOCK) ON m.number = p.number AND batchtype = 'pu'
WHERE m.customer IN (Select customerid from fact where customgroupid = @CustGroupID)
AND n.created BETWEEN dbo.F_START_OF_MONTH (GETDATE())  AND dbo.F_END_OF_MONTH (GETDATE())
AND user0 = 'iconnect24' AND action = 'eml' AND result = 'eml-ok' --AND p1.number IS NOT NULL
AND p.datepaid  BETWEEN dbo.F_START_OF_MONTH (GETDATE())  AND dbo.F_END_OF_MONTH (GETDATE())
...
AND user0 = 'iconnect24' AND action = 'eml' AND result = 'eml-ok' --AND p1.number IS NOT NULL
AND p.datepaid BETWEEN dbo.F_START_OF_MONTH (GETDATE())  AND dbo.F_END_OF_MONTH (GETDATE())
) AS [Total Dollars Collected W/In 5 Days of email- All Sources]
...
AND psae.Occurred BETWEEN CAST(dbo.F_START_OF_MONTH (GETDATE()) AS DATE) AND CAST(dbo.F_END_OF_MONTH(GETDATE()) AS DATE)) AS [Web portal entries]
...
AND n.created BETWEEN dbo.F_START_OF_MONTH (GETDATE())  AND dbo.F_END_OF_MONTH (GETDATE())
AND user0 = 'iconnect24' AND action = 'eml' AND result = 'eml-ok'))
AS [# of Web portal Payments within 5 days of email]
...
, ISNULL( (SELECT SUM(@@rowcount) FROM (SELECT count(*) AS numaccts FROM pdc p WITH (NOLOCK) WHERE customer IN (Select customerid from fact where customgroupid = @CustGroupID) AND ApprovedBy = 'payweb' 
		AND p.entered BETWEEN dbo.F_START_OF_MONTH (GETDATE())  AND dbo.F_END_OF_MONTH (GETDATE())
		...
		AND p.DateEntered BETWEEN dbo.F_START_OF_MONTH (GETDATE())  AND dbo.F_END_OF_MONTH (GETDATE())
...
AND CAST(p.datepaid AS DATE) BETWEEN CAST(dbo.F_START_OF_MONTH(GETDATE()) AS DATE) AND CAST(dbo.F_END_OF_MONTH(GETDATE()) AS DATE)) AS pw
