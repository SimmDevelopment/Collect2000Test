...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[Dashboard_GetAllPerformance](@Date DATETIME, @MonthsBack INT)
AS
BEGIN
   ...
   SELECT MONTH(P.entered),
      YEAR(P.entered),
      SUM(P.TotalPaid) AS Collections,
      SUM(P.FEE1+P.FEE2+P.FEE3+P.FEE4+P.FEE5+P.FEE6+P.FEE7+P.FEE8+P.FEE9+P.FEE10) AS Fees
   FROM Payhistory P WITH(NOLOCK) 
   JOIN Customer C ON C.customer = P.customer
   WHERE C.[status] = 'active'
