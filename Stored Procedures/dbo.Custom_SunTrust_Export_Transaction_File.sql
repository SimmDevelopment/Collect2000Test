...
-- =============================================
Create PROCEDURE [dbo].[Custom_SunTrust_Export_Transaction_File] 
...
FROM    Custom_SunTrust_Temp_Address cubta WITH ( NOLOCK )

SELECT  AgencyId, LocationCode, AcctNumber, TransCode, TransDate, ContactId, ContactRecordType, ExternalKey, ActionCode,
        NickName, EmailAddress, EmailType, EmailAvailability, PreferredEmailAddressInd, ChangeDate
FROM    Custom_SunTrust_Temp_Emails cubte WITH ( NOLOCK )
...
FROM    Custom_SunTrust_Temp_Phones cubtp WITH ( NOLOCK )
...
FROM    Custom_SunTrust_Temp_TimeToCall cubtttc WITH ( NOLOCK )
