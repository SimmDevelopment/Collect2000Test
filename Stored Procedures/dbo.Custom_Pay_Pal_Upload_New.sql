...
1/22/2016 BGM Changed Payment buckets to value '00000000.00' per scott at paypal.
...
09/01/2021 BGM updated total paid to return a negative sign or 0 depending on amount for first position.
*/
...
set @startDate='20210831'
set @endDate='20210831'
--set @endDate='12/20/2006 12:15:00'
set @customer='0002462|'
set @invoice = '201333'
exec Custom_Pay_Pal_Upload @startDate=@startDate,@endDate=@endDate,@customer=@customer,@invoice = @invoice
*/
CREATE                        PROCEDURE [dbo].[Custom_Pay_Pal_Upload_New]
	 @startDate datetime,
	 @endDate datetime,
...
(SELECT distinct --Note record select top 10 * from Deceased
	...
	case WHEN customer = '0001220' THEN 
		CASE when m.status like 'B%' then 'DBK'
			...
			ELSE 'PTP'
		END
		WHEN customer in ('0001256','0001257','0001258','0002462') THEN
	...
	CASE WHEN m.customer = '0001220' THEN
		...
		CASE WHEN m.status like 'B%' THEN 'BANKRUPTCY'
			...
			WHEN m.STATUS = 'PCM' THEN 'COMPLAINT'
			ELSE 'PROMISE TO PAY'--substring(n.comment,0,140)
		end
	END AS [Narrative1],
	--substring(n.comment,140,140) AS [Narrative2],
...
WHERE m.status in ('AEX', 'DEC','CCS','CAD', 'DPV', 'PLV', 'RSK', 'FRD', 'CND', 'MIL', 'DIP', 'POA', 'WVI', 'OOS', 'DIN','PCL','CMP')
...
LEFT OUTER JOIN master m with (nolock) ON t.number = m.number
LEFT OUTER JOIN statushistory s WITH(NOLOCK) ON s.accountid= m.number
WHERE m.status in ('VRB', 'BKO', 'BKY', 'B07', 'B11', 'B13', 'LCP', 'VCD', 'pdc', 'pcc', 'ppa', 'nsf', 'dcc', 'dbd', 'npc', 'hot', 'mhd', 'nhd', 'DIS') 
AND m.customer IN (select string from dbo.CustomStringToSet(@customer, '|'))
)
