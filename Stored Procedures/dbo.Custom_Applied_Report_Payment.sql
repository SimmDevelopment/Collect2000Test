...
*/
CREATE           PROCEDURE [dbo].[Custom_Applied_Report_Payment]
	 @invoice varchar(255),
...
SELECT DISTINCT --Header Record
	...
	case
		when m.customer= @customer then 'SIMM Associates Inc.'
		else ''
	end AS [AgencyName],
	--Rec1
	(select count(*) FROM PayHistory p with (nolock) JOIN Master m2 with (nolock) ON m2.Number=p.Number WHERE invoice  in (select string from dbo.CustomStringToSet(@invoice, '|')) AND p.batchtype IN ('PU', 'PA', 'PUR', 'PAR') AND m2.customer=m.customer) /*IN (select string from dbo.CustomStringToSet(@customer, '|')))*/ AS [NumRec1],
	(select SUM(dbo.Custom_ReversePaymentSign(dbo.Custom_CalculatePaymentTotalPaid (p.uid), p.batchtype)) FROM PayHistory p with (nolock) JOIN Master m2 with (nolock) ON m2.Number=p.Number WHERE invoice  in (select string from dbo.CustomStringToSet(@invoice, '|')) AND p.batchtype IN ('PU', 'PA', 'PUR', 'PAR') AND m2.customer=m.customer) /*IN (select string from dbo.CustomStringToSet(@customer, '|')))*/ AS [TotalPay],
	--Rec2
...
where m.customer = @customer and m.customer is not null
