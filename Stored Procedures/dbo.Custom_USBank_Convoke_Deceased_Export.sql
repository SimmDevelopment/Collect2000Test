...
-- =============================================
CREATE PROCEDURE [dbo].[Custom_USBank_Convoke_Deceased_Export]
...
CASE WHEN serviceid = '5024' AND (SELECT TOP 1 requestid FROM Services_TLO_Bankruptcy WHERE sh.RequestID = RequestID) IS NOT NULL THEN 'Postive'
WHEN serviceid = '5025' AND (SELECT TOP 1 requestid FROM Services_TLO_Deceased WHERE sh.RequestID = RequestID) IS NOT NULL THEN 'Postive' ELSE 'Negative' END  AS ScrubResult, 
