...
-- =============================================
CREATE PROCEDURE [dbo].[Custom_Fortress_Demographics_File]
...
BEGIN
	...
	SELECT dbo.date(GETDATE()) AS effdate, '30' AS SRC, '39' AS PTADR, m.ssn AS PRSID, 'L' AS DCADR, d.Street1 AS STRADR1, d.Street2 AS STRADR2,
		'' AS STRADR3, d.City AS CT, d.State AS DOMST, d.Zipcode AS ZIPCDE, '' AS FGNST, '' AS FGNCNY, '' AS FGNCTR, 'H' AS PHNTYPE, 
...
WHERE dbo.date(ah.DateChanged) BETWEEN dbo.date(@startDate) AND dbo.date(@endDate) AND customer IN ('0001034')
