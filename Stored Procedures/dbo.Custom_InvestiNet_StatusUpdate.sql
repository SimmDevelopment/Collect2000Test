...
-- =============================================
CREATE PROCEDURE [dbo].[Custom_InvestiNet_StatusUpdate]
...
BEGIN
	...
	CASE WHEN CONVERT(VARCHAR(10), m.received, 101) = CONVERT(VARCHAR(10), GETDATE(), 101) THEN '101000' 
	WHEN CONVERT(VARCHAR(10), m.received, 101) = CONVERT(VARCHAR(10), GETDATE(), 101) THEN '191000'
	...
	WHEN status IN ('RCL', 'CCR') THEN '900100' ELSE '112010' END AS placedetail_status, 
...
AND (CASE WHEN CONVERT(VARCHAR(10), m.received, 101) = CONVERT(VARCHAR(10), GETDATE(), 101) THEN '101000' 
	...
	WHEN status = 'B07' THEN '901007' WHEN status = 'B11' THEN '901011' WHEN status = 'B12' THEN '901012' WHEN status = 'B13' THEN '901013'
	WHEN status = 'BKY' THEN '901007' WHEN status = 'DEC' THEN '902000' WHEN status = 'RSK' THEN '112070' WHEN status IN ('CND', 'CAD') THEN '910120' WHEN status in ('AEX') THEN '910180' 
	WHEN status IN ('RCL', 'CCR') THEN '900100' ELSE '112010' END <> (SELECT TOP 1 statuscode FROM dbo.Custom_InvestiNet_Status_Codes WITH (NOLOCK) WHERE m.id1 = DataID ORDER BY statusdate DESC)
...
SELECT id1 AS data_id, CASE WHEN CONVERT(VARCHAR(10), m.received, 101) = CONVERT(VARCHAR(10), GETDATE(), 101)  THEN '101000' 
	...
	WHEN status = 'B07' THEN '901007' WHEN status = 'B11' THEN '901011' WHEN status = 'B12' THEN '901012' WHEN status = 'B13' THEN '901013'
	WHEN status = 'BKY' THEN '901007' WHEN status = 'DEC' THEN '902000' WHEN status = 'RSK' THEN '112070' WHEN status IN ('CND', 'CAD') THEN '910120' WHEN status in ('AEX') THEN '910180' 
	WHEN status IN ('RCL', 'CCR') THEN '900100' ELSE '112010' END AS status_code, dbo.date(GETDATE()) AS status_date
