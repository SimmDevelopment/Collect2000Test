...
--exec Custom_HSBC_Vegas_Retail_Export_Maintenance '2010-08-16 16:22:46.500', '2010-08-17 16:22:46.500', '0001013', '23174'

--ALTER  the procedure.
CREATE      PROCEDURE [dbo].[Custom_HSBC_Vegas_Retail_Export_Maintenance]
@BeginDate datetime,
@EndDate datetime,
...
AS
BEGIN
	CREATE TABLE #HSBCVegasReturns(number int)
	...
	CREATE TABLE #HSBCVegas_TempUploadMaintenance
	(OrderByFlag int,
...
SELECT CASE WHEN ph.batchtype in ('PU','PA') THEN REPLACE(CONVERT(char, GetDate(), 111), '/', '') ELSE (SELECT datepaid FROM payhistory WHERE uid = ph.reverseofuid) END as TransDate,
       ...
       CASE
	    WHEN ph.batchtype IN ('PU','PA') THEN '52'
            WHEN ph.batchtype IN ('PUR','PAR') THEN '53'
       END AS TransCode,
       TransAmt = 
           CASE            
...
from #HSBCVegas_TempUploadFinancials
	...
	INSERT INTO #HSBCVegas_TempUploadMaintenance
	(OrderByFlag,
	TransDate,
	TransTime,
	Account,
	...
	INNER JOIN Debtors d WITH (NOLOCK) ON PH.DebtorID = D.DebtorID
	INNER JOIN Master  m WITH (NOLOCK)
	ON d.number = m.number
	INNER JOIN customer c WITH(NOLOCK)
	ON c.customer = m.customer
	...
	INSERT INTO #HSBCVegasReturns(number)
	SELECT m.number
	FROM master m WITH(NOLOCK)
	INNER JOIN customer c WITH(NOLOCK)
	...
	m.status IN('B07','B11','B13','BKY','DEC','FRD', 'CND', 'CAD', 'DSP','SIF','PIF','AEX') AND
	m.Qlevel NOT IN('999')
...
/*
	...
	WHERE number IN(SELECT number from #HSBCVegasReturns)

	-- Insert a Note Showing the return of the account.
	INSERT INTO Notes(number,created,user0,action,result,comment)
	SELECT t.number,@runDate,'EXG','+++++','+++++','Account was Returned in a HSBC Vegas Maintenance Export file.'--'Account was returned to HSBCVegas during the Maintenance Export process.'
	FROM #HSBCVegasReturns t 
*/
	...
	INNER JOIN customer c WITH(NOLOCK)
	ON c.customer = m.customer
	WHERE m.number IN (SELECT number FROM #HSBCVegasReturns) AND m.status IN('B07','B11','B13','BKY','DEC','FRD', 'CND', 'CAD', 'DSP', 'AEX')
	...
	INNER JOIN customer c WITH(NOLOCK)
	ON c.customer = m.customer 
	WHERE (m.number IN(SELECT number FROM #HSBCVegasReturns)) and
          (m.Status in ('PIF', 'SIF'))  
             --and (datediff(d, m.LastPaid, (GetDate())) >= 30)
...
/*  HSBC vegas does not do soft recalls.
	-- SOFT RECALLS HOLD
	INSERT INTO #HSBCVegas_TempUploadMaintenance
	(OrderByFlag,
	TransDate,
...
SELECT * FROM #HSBCVegas_TempUploadMaintenance
