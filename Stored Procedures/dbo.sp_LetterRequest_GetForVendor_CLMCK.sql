...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[sp_LetterRequest_GetForVendor_CLMCK] (@LetterID INT,
@ThroughDate DATETIME,
@IncludeErrors BIT = 0)
WITH RECOMPILE

--exec sp_LetterRequest_GetForVendor_CLMCK 488, '20220606'
...
--		06/06/2022 BGM Added code for the Caim Check List letters to be sent with the Attorney or Executor names and addresses.
	BEGIN
	...
	WITH CTE_LetterAccountData AS (SELECT LR.LetterRequestId,
	LR.AccountID,
	LR.LetterCode,
	LR.AmountDue,
	LR.DueDate,
	...
	A.TotalAdjustment9,
	A.TotalAdjustment10,
	CASE WHEN d.debtorid IN (SELECT Debtorattorneys.DebtorID FROM [DebtorAttorneys] WITH (NOLOCK) WHERE [DebtorAttorneys].[DebtorID] = d.[DebtorID] AND	(
	([DebtorAttorneys].[Addr1] IS NOT NULL AND ([DebtorAttorneys].[Addr1] <> '' OR [DebtorAttorneys].[Addr1] IS NULL))
		AND ([DebtorAttorneys].[City] IS NOT NULL AND ([DebtorAttorneys].[City] <> '' OR [DebtorAttorneys].[City] IS NULL))
		AND ([DebtorAttorneys].[State] IS NOT NULL AND ([DebtorAttorneys].[State] <> '' OR [DebtorAttorneys].[State] IS NULL))
		AND ([DebtorAttorneys].[Zipcode] IS NOT NULL AND ([DebtorAttorneys].[Zipcode] <> '' OR [DebtorAttorneys].[Zipcode] IS NULL))
		AND ([DebtorAttorneys].[Firm] IS NOT NULL AND ([DebtorAttorneys].[Name] <> '' OR [DebtorAttorneys].[Name] IS NULL))))
	THEN 1 WHEN d.debtorid IN (SELECT Deceased.DebtorID FROM [Deceased] WITH (NOLOCK) WHERE [Deceased].[DebtorID] = d.[DebtorID] AND (
	([Deceased].[Executor] IS NOT NULL AND ([Deceased].[Executor] <> '' OR [Deceased].[Executor] IS NULL))
		AND ([Deceased].[ExecutorStreet1] IS NOT NULL AND ([Deceased].[ExecutorStreet1] <> '' OR [Deceased].[ExecutorStreet1] IS NULL))
		AND ([Deceased].[ExecutorCity] IS NOT NULL AND ([Deceased].[ExecutorCity] <> '' OR [Deceased].[ExecutorCity] IS NULL))
		AND ([Deceased].[ExecutorState] IS NOT NULL	AND ([Deceased].[ExecutorState] <> '' OR [Deceased].[ExecutorState] IS NULL))
		AND ([Deceased].[ExecutorZipcode] IS NOT NULL AND ([Deceased].[ExecutorZipcode] <> '' OR [Deceased].[ExecutorZipcode] IS NULL))))
	THEN 2 ELSE 3 END AS ClmCheckAddr
	FROM dbo.letter AS L
		 INNER JOIN dbo.LetterRequest AS LR ON L.LetterID = LR.LetterID
		 INNER JOIN dbo.LetterRequest_Account AS LRA ON LRA.AccountID = LR.AccountID AND LRA.LetterRequestId = LR.LetterRequestId
		 ...
		 INNER JOIN dbo.Debtors AS D ON D.DebtorId = LRR.DebtorId
		 INNER JOIN dbo.master AS M ON LR.AccountID = M.number
		 LEFT JOIN (SELECT number,
		 SUM(CASE WHEN batchtype LIKE 'p%' AND
		 batchtype LIKE '%r' THEN -totalpaid WHEN batchtype LIKE 'p%' AND
		 batchtype NOT LIKE '%r' THEN totalpaid ELSE 0 END) AS TotalPaidAmount,
		 SUM(CASE WHEN batchtype LIKE 'p%' AND
		 batchtype LIKE '%r' THEN -paid1 WHEN batchtype LIKE 'p%' AND
		 batchtype NOT LIKE '%r' THEN paid1 ELSE 0 END) AS TotalPaid1,
		 SUM(CASE WHEN batchtype LIKE 'p%' AND
		 batchtype LIKE '%r' THEN -paid2 WHEN batchtype LIKE 'p%' AND
		 batchtype NOT LIKE '%r' THEN paid2 ELSE 0 END) AS TotalPaid2,
		 SUM(CASE WHEN batchtype LIKE 'p%' AND
		 batchtype LIKE '%r' THEN -paid3 WHEN batchtype LIKE 'p%' AND
		 batchtype NOT LIKE '%r' THEN paid3 ELSE 0 END) AS TotalPaid3,
		 SUM(CASE WHEN batchtype LIKE 'p%' AND
		 batchtype LIKE '%r' THEN -paid4 WHEN batchtype LIKE 'p%' AND
		 batchtype NOT LIKE '%r' THEN paid4 ELSE 0 END) AS TotalPaid4,
		 SUM(CASE WHEN batchtype LIKE 'p%' AND
		 batchtype LIKE '%r' THEN -paid5 WHEN batchtype LIKE 'p%' AND
		 batchtype NOT LIKE '%r' THEN paid5 ELSE 0 END) AS TotalPaid5,
		 SUM(CASE WHEN batchtype LIKE 'p%' AND
		 batchtype LIKE '%r' THEN -paid6 WHEN batchtype LIKE 'p%' AND
		 batchtype NOT LIKE '%r' THEN paid6 ELSE 0 END) AS TotalPaid6,
		 SUM(CASE WHEN batchtype LIKE 'p%' AND
		 batchtype LIKE '%r' THEN -paid7 WHEN batchtype LIKE 'p%' AND
		 batchtype NOT LIKE '%r' THEN paid7 ELSE 0 END) AS TotalPaid7,
		 SUM(CASE WHEN batchtype LIKE 'p%' AND
		 batchtype LIKE '%r' THEN -paid8 WHEN batchtype LIKE 'p%' AND
		 batchtype NOT LIKE '%r' THEN paid8 ELSE 0 END) AS TotalPaid8,
		 SUM(CASE WHEN batchtype LIKE 'p%' AND
		 batchtype LIKE '%r' THEN -paid9 WHEN batchtype LIKE 'p%' AND
		 batchtype NOT LIKE '%r' THEN paid9 ELSE 0 END) AS TotalPaid9,
		 SUM(CASE WHEN batchtype LIKE 'p%' AND
		 batchtype LIKE '%r' THEN -paid10 WHEN batchtype LIKE 'p%' AND
		 batchtype NOT LIKE '%r' THEN paid10 ELSE 0 END) AS TotalPaid10,
		 SUM(CASE WHEN batchtype LIKE 'p%' AND
		 batchtype LIKE '%r' THEN -OverPaidAmt WHEN batchtype LIKE 'p%' AND
		 batchtype NOT LIKE '%r' THEN OverPaidAmt ELSE 0 END) AS TotalOverPaidAmount,
		 SUM(CASE WHEN batchtype LIKE 'd%' AND
		 batchtype LIKE '%r' THEN totalpaid WHEN batchtype LIKE 'd%' AND
		 batchtype NOT LIKE '%r' THEN -totalpaid ELSE 0 END) AS TotalAdjustmentAmount,
		 SUM(CASE WHEN batchtype LIKE 'd%' AND
		 batchtype LIKE '%r' THEN paid1 WHEN batchtype LIKE 'd%' AND
		 batchtype NOT LIKE '%r' THEN -paid1 ELSE 0 END) AS TotalAdjustment1,
		 SUM(CASE WHEN batchtype LIKE 'd%' AND
		 batchtype LIKE '%r' THEN paid2 WHEN batchtype LIKE 'd%' AND
		 batchtype NOT LIKE '%r' THEN -paid2 ELSE 0 END) AS TotalAdjustment2,
		 SUM(CASE WHEN batchtype LIKE 'd%' AND
		 batchtype LIKE '%r' THEN paid3 WHEN batchtype LIKE 'd%' AND
		 batchtype NOT LIKE '%r' THEN -paid3 ELSE 0 END) AS TotalAdjustment3,
		 SUM(CASE WHEN batchtype LIKE 'd%' AND
		 batchtype LIKE '%r' THEN paid4 WHEN batchtype LIKE 'd%' AND
		 batchtype NOT LIKE '%r' THEN -paid4 ELSE 0 END) AS TotalAdjustment4,
		 SUM(CASE WHEN batchtype LIKE 'd%' AND
		 batchtype LIKE '%r' THEN paid5 WHEN batchtype LIKE 'd%' AND
		 batchtype NOT LIKE '%r' THEN -paid5 ELSE 0 END) AS TotalAdjustment5,
		 SUM(CASE WHEN batchtype LIKE 'd%' AND
		 batchtype LIKE '%r' THEN paid6 WHEN batchtype LIKE 'd%' AND
		 batchtype NOT LIKE '%r' THEN -paid6 ELSE 0 END) AS TotalAdjustment6,
		 SUM(CASE WHEN batchtype LIKE 'd%' AND
		 batchtype LIKE '%r' THEN paid7 WHEN batchtype LIKE 'd%' AND
		 batchtype NOT LIKE '%r' THEN -paid7 ELSE 0 END) AS TotalAdjustment7,
		 SUM(CASE WHEN batchtype LIKE 'd%' AND
		 batchtype LIKE '%r' THEN paid8 WHEN batchtype LIKE 'd%' AND
		 batchtype NOT LIKE '%r' THEN -paid8 ELSE 0 END) AS TotalAdjustment8,
		 SUM(CASE WHEN batchtype LIKE 'd%' AND
		 batchtype LIKE '%r' THEN paid9 WHEN batchtype LIKE 'd%' AND
		 batchtype NOT LIKE '%r' THEN -paid9 ELSE 0 END) AS TotalAdjustment9,
		 SUM(CASE WHEN batchtype LIKE 'd%' AND
		 batchtype LIKE '%r' THEN paid10 WHEN batchtype LIKE 'd%' AND
		 batchtype NOT LIKE '%r' THEN -paid10 ELSE 0 END) AS TotalAdjustment10
		 FROM dbo.payhistory
		 GROUP BY number) AS A ON M.number = A.number
	WHERE LR.LetterID = @LetterID AND
	CAST(LR.DateRequested as DATE) <= CAST(@ThroughDate AS DATE) AND
	(
	...
	CTE_LinkedList AS (SELECT LR.LetterRequestId,
	LR.AccountID,
	LR.LetterCode,
	LR.AmountDue,
	LR.DueDate,
	...
	FROM dbo.LetterRequest AS LR
		 ...
		 ON LR.LetterRequestId = LRR.LetterRequestId
		 INNER JOIN dbo.Debtors AS D ON D.DebtorId = LRR.DebtorId
		 INNER JOIN dbo.letter AS L ON L.linkedLetter = 1 AND
		 L.LetterID = LR.LetterID
	WHERE LR.LetterID = @LetterID AND
	CAST(LR.DateRequested AS DATE) <= CAST(@ThroughDate AS DATE) AND
	COALESCE(LR.DateProcessed, '1/1/1753 12:00:00') = '1/1/1753 12:00:00' AND
	LR.Deleted = 0 AND
	LR.AddEditMode = 0 AND
	LR.Suspend = 0 AND
	LR.Edited = 0 AND
	...
	CTE_HeaderData AS (SELECT HD.LetterRequestId,
	HD.AccountID,
	HD.LetterCode,
	HD.AmountDue,
	HD.DueDate,
	...
	HD.InterestRate,
	HD.ClmCheckAddr
	FROM CTE_LetterAccountData AS HD
	UNION ALL
	SELECT LL.LetterRequestId,
	LL.AccountID,
	LL.LetterCode,
	LL.AmountDue,
	LL.DueDate,
	...
	M.InterestRate,
	LL.ClmCheckAddr
	FROM CTE_LinkedList AS LL
		 INNER JOIN dbo.master AS M ON M.Link = LL.Link AND
		 LL.AccountID != M.number
		 INNER JOIN dbo.LetterRequest_Account AS LRA ON LRA.LetterRequestId = LL.LetterRequestId AND LRA.AccountID = M.number
		 LEFT JOIN (SELECT number,
		 SUM(CASE WHEN batchtype LIKE 'p%' AND
		 batchtype LIKE '%r' THEN -totalpaid WHEN batchtype LIKE 'p%' AND
		 batchtype NOT LIKE '%r' THEN totalpaid ELSE 0 END) AS TotalPaidAmount,
		 ...
		 FROM dbo.payhistory
		 GROUP BY number) AS A ON M.number = A.number
	WHERE M.Qlevel NOT LIKE '99[89]'),
	CTE_ExtraData AS (SELECT ED.number,
	MAX(CASE ED.extracode WHEN 'L1' THEN ED.line1 ELSE '' END) AS L1_Line1,
	MAX(CASE ED.extracode WHEN 'L1' THEN ED.line2 ELSE '' END) AS L1_Line2,
	MAX(CASE ED.extracode WHEN 'L1' THEN ED.line3 ELSE '' END) AS L1_Line3,
	MAX(CASE ED.extracode WHEN 'L1' THEN ED.line4 ELSE '' END) AS L1_Line4,
	MAX(CASE ED.extracode WHEN 'L1' THEN ED.line5 ELSE '' END) AS L1_Line5,
	MAX(CASE ED.extracode WHEN 'L2' THEN ED.line1 ELSE '' END) AS L2_Line1,
	MAX(CASE ED.extracode WHEN 'L2' THEN ED.line2 ELSE '' END) AS L2_Line2,
	MAX(CASE ED.extracode WHEN 'L2' THEN ED.line3 ELSE '' END) AS L2_Line3,
	MAX(CASE ED.extracode WHEN 'L2' THEN ED.line4 ELSE '' END) AS L2_Line4,
	MAX(CASE ED.extracode WHEN 'L2' THEN ED.line5 ELSE '' END) AS L2_Line5,
	MAX(CASE ED.extracode WHEN 'L3' THEN ED.line1 ELSE '' END) AS L3_Line1,
	MAX(CASE ED.extracode WHEN 'L3' THEN ED.line2 ELSE '' END) AS L3_Line2,
	MAX(CASE ED.extracode WHEN 'L3' THEN ED.line3 ELSE '' END) AS L3_Line3,
	MAX(CASE ED.extracode WHEN 'L3' THEN ED.line4 ELSE '' END) AS L3_Line4,
	MAX(CASE ED.extracode WHEN 'L3' THEN ED.line5 ELSE '' END) AS L3_Line5,
	MAX(CASE ED.extracode WHEN 'L4' THEN ED.line1 ELSE '' END) AS L4_Line1,
	MAX(CASE ED.extracode WHEN 'L4' THEN ED.line2 ELSE '' END) AS L4_Line2,
	MAX(CASE ED.extracode WHEN 'L4' THEN ED.line3 ELSE '' END) AS L4_Line3,
	MAX(CASE ED.extracode WHEN 'L4' THEN ED.line4 ELSE '' END) AS L4_Line4,
	MAX(CASE ED.extracode WHEN 'L4' THEN ED.line5 ELSE '' END) AS L4_Line5,
	MAX(CASE ED.extracode WHEN 'L5' THEN ED.line1 ELSE '' END) AS L5_Line1,
	MAX(CASE ED.extracode WHEN 'L5' THEN ED.line2 ELSE '' END) AS L5_Line2,
	MAX(CASE ED.extracode WHEN 'L5' THEN ED.line3 ELSE '' END) AS L5_Line3,
	MAX(CASE ED.extracode WHEN 'L5' THEN ED.line4 ELSE '' END) AS L5_Line4,
	MAX(CASE ED.extracode WHEN 'L5' THEN ED.line5 ELSE '' END) AS L5_Line5
	FROM dbo.extradata AS ED
	WHERE ED.extracode IN ('L1', 'L2', 'L3', 'L4', 'L5')
	GROUP BY ED.number)
	...
	--If letter is a claim checklist send to Atorney first then Executor
	CASE WHEN @LetterID IN (397, 481, 549) THEN  CASE WHEN m.ClmCheckAddr = 1 THEN ISNULL(da.Name, ISNULL(da.Firm, '')) WHEN m.ClmCheckAddr = 2 THEN ISNULL(dc.Executor, '') + ' ' + ISNULL(dc.Executor, '') END
	ELSE CASE WHEN M.AltRecipient = 1 THEN ISNULL(M.AltName, '') WHEN r.letterstoatty = 1 THEN ISNULL(da.Name, '') ELSE ISNULL(M.DebtorName, '') END END AS Name,
	...
	--If letter is a claim checklist send to Atorney first then Executor
	CASE WHEN @LetterID IN (397, 481, 549) THEN CASE WHEN m.ClmCheckAddr = 1 THEN ISNULL(da.Name, ISNULL(da.Firm, '')) WHEN m.ClmCheckAddr = 2 THEN ISNULL(dc.Executor, '') + ' ' + ISNULL(dc.Executor, '') END
	ELSE CASE WHEN M.AltRecipient = 1 THEN LTRIM(ISNULL(dbo.GetFirstName(M.AltName) + ' ' + dbo.GetLastName(M.AltName), ''))
	--changed to include debtor name with c/o atty name
	WHEN r.letterstoatty = 1 THEN CASE WHEN sd.isBusiness = 1 THEN M.businessname + ' C/O ' + LTRIM(ISNULL(dbo.GetFirstName(da.Name) + ' ' + dbo.GetLastName(da.Name), '')) ELSE LTRIM(ISNULL(CASE M.isParsed WHEN 1 THEN M.firstName ELSE dbo.GetFirstName(sd.Name) END + ' ' + CASE sd.isParsed WHEN 1 THEN sd.lastName ELSE dbo.GetLastName(sd.Name) END, ''))
	...
	WHEN m.customer IN ('0002468', '0002469') AND M.lettercode = 'PPWC1' AND M.JobName <> '' THEN M.JobName ELSE LTRIM(ISNULL(CASE sd.isParsed WHEN 1 THEN sd.firstName ELSE dbo.GetFirstName(sd.Name) END + ' ' + CASE sd.isParsed WHEN 1 THEN sd.lastName ELSE dbo.GetLastName(sd.Name) END, '')) END END AS [FirstNameFirst],

	--If letter is a claim checklist send to Atorney first then Executor
	CASE WHEN @LetterID IN (397, 481, 549) THEN CASE WHEN m.ClmCheckAddr = 1 THEN ISNULL(dbo.GetLastName(da.Name), '') WHEN m.ClmCheckAddr = 2 THEN ISNULL(dbo.GetLastName(dc.Executor), '') END
	ELSE CASE WHEN M.AltRecipient = 1 THEN ISNULL(dbo.GetLastName(M.AltName), '') WHEN r.letterstoatty = 1 THEN ISNULL(da.Name, '') ELSE ISNULL(CASE M.isParsed WHEN 1 THEN M.lastName ELSE dbo.GetLastName(M.DebtorName) END, '') END END AS LastName,

	--If letter is a claim checklist send to Atorney first then Executor
	CASE WHEN @LetterID IN (397, 481, 549) THEN CASE WHEN m.ClmCheckAddr = 1 THEN ISNULL(dbo.GetFirstName(da.Name), '') WHEN m.ClmCheckAddr = 2 THEN ISNULL(dbo.GetFirstName(dc.Executor), '') END
	ELSE CASE WHEN M.AltRecipient = 1 THEN ISNULL(dbo.GetFirstName(M.AltName), '') WHEN r.letterstoatty = 1 THEN ISNULL(da.Name, '') ELSE ISNULL(CASE M.isParsed WHEN 1 THEN M.firstName ELSE dbo.GetFirstName(M.DebtorName) END, '') END END AS FirstName,

	ISNULL(M.OtherName, '') AS Other,

	CASE WHEN @LetterID IN (397, 481, 549) THEN CASE WHEN m.ClmCheckAddr = 1 THEN ISNULL(da.Addr1, '') WHEN m.ClmCheckAddr = 2 THEN ISNULL(dc.ExecutorStreet1, '') END
	ELSE CASE WHEN M.AltRecipient = 1 THEN ISNULL(M.AltStreet1, '') WHEN r.letterstoatty = 1 THEN ISNULL(da.Addr1, '') ELSE ISNULL(M.Street1, '') END END AS Street1,

	CASE WHEN @LetterID IN (397, 481, 549) THEN CASE WHEN m.ClmCheckAddr = 1 THEN ISNULL(da.Addr2, '') WHEN m.ClmCheckAddr = 2 THEN ISNULL(dc.ExecutorStreet2, '') END
	ELSE CASE WHEN M.AltRecipient = 1 THEN ISNULL(M.AltStreet2, '') WHEN r.letterstoatty = 1 THEN ISNULL(da.Addr2, '') ELSE ISNULL(M.Street2, '') END END AS Street2,

	CASE WHEN @LetterID IN (397, 481, 549) THEN CASE WHEN m.ClmCheckAddr = 1 THEN ISNULL(da.City, '') WHEN m.ClmCheckAddr = 2 THEN ISNULL(dc.ExecutorCity, '') END
	ELSE CASE WHEN M.AltRecipient = 1 THEN ISNULL(M.AltCity, '') WHEN r.letterstoatty = 1 THEN ISNULL(da.City, '') ELSE ISNULL(M.City, '') END END AS City,

	CASE WHEN @LetterID IN (397, 481, 549) THEN CASE WHEN m.ClmCheckAddr = 1 THEN ISNULL(da.State, '') WHEN m.ClmCheckAddr = 2 THEN ISNULL(dc.ExecutorCity, '') END
	ELSE CASE WHEN M.AltRecipient = 1 THEN ISNULL(M.AltState, '') WHEN r.letterstoatty = 1 THEN ISNULL(da.State, '') ELSE ISNULL(M.State, '') END END AS State,

	CASE WHEN @LetterID IN (397, 481, 549) THEN CASE WHEN m.ClmCheckAddr = 1 THEN ISNULL(da.Zipcode, '') WHEN m.ClmCheckAddr = 2 THEN ISNULL(dc.ExecutorZipcode, '') END
	ELSE CASE WHEN M.AltRecipient = 1 THEN ISNULL(M.AltZipcode, '') WHEN r.letterstoatty = 1 THEN ISNULL(da.Zipcode, '') ELSE ISNULL(M.Zipcode, '') END END AS Zipcode,

	CASE WHEN @LetterID IN (397, 481, 549) THEN CASE WHEN m.ClmCheckAddr = 1 THEN CASE WHEN (LEN(ISNULL(da.City, '')) > 0) THEN ISNULL(da.City, '') + ', ' + ISNULL(da.State, '') + ' ' + ISNULL(da.Zipcode, '') ELSE ISNULL(da.State, '') + ' ' + ISNULL(da.Zipcode, '') END
	WHEN m.ClmCheckAddr = 2 THEN CASE WHEN (LEN(ISNULL(dc.ExecutorCity, '')) > 0) THEN ISNULL(dc.ExecutorCity, '') + ', ' + ISNULL(dc.ExecutorState, '') + ' ' + ISNULL(dc.ExecutorZipcode, '') ELSE ISNULL(dc.ExecutorState, '') + ' ' + ISNULL(dc.ExecutorZipcode, '') END END
	ELSE CASE WHEN M.AltRecipient = 1 THEN CASE WHEN (LEN(ISNULL(M.AltCity, '')) > 0) THEN ISNULL(M.AltCity, '') + ', ' + ISNULL(M.AltState, '') + ' ' + ISNULL(M.AltZipcode, '') ELSE ISNULL(M.AltState, '') + ' ' + ISNULL(M.AltZipcode, '') END 
	WHEN r.letterstoatty = 1 THEN CASE WHEN (LEN(ISNULL(da.City, '')) > 0) THEN ISNULL(da.City, '') + ', ' + ISNULL(da.State, '') + ' ' + ISNULL(da.Zipcode, '') ELSE ISNULL(da.State, '') + ' ' + ISNULL(da.Zipcode, '') END 
	ELSE CASE WHEN (LEN(ISNULL(M.City, '')) > 0) THEN ISNULL(M.City, '') + ', ' + ISNULL(M.State, '') + ' ' + ISNULL(M.Zipcode, '') ELSE ISNULL(M.State, '') + ' ' + ISNULL(M.Zipcode, '') END END END AS CSZ,
	...
	CASE WHEN M.AltRecipient = 1 THEN ISNULL(M.AltFax, '') WHEN r.letterstoatty = 1 THEN ISNULL(da.Fax, '') ELSE ISNULL(M.Fax, '') END AS Fax,
...
CASE WHEN M.lettercode IN ('mosta', 'mostp', 'ny01') THEN '$' + CONVERT(VARCHAR(50), ISNULL(ABS(m.Paid10), 0), 1) WHEN (ISNULL(m.Paid10, 0) >= 0) THEN '$' + CONVERT(VARCHAR(50), ISNULL(m.Paid10, 0), 1) ELSE '($' + CONVERT(VARCHAR(50), ISNULL(ABS(m.Paid10), 0), 1) + ')' END AS [Paid10],
	...
	CASE WHEN (ISNULL(M.current0, 0) >= 0) THEN '$' + CONVERT(VARCHAR(50), ISNULL(M.current0, 0), 1) ELSE '($' + CONVERT(VARCHAR(50), ISNULL(ABS(M.current0), 0), 1) + ')' END AS [Current],
...
WHERE CustomGroupID = 207) THEN m.previouscreditor ELSE ISNULL(cu.Name, '')  END AS [Customer.Name],
	...
	CASE WHEN COALESCE(lb.current0, M.current0, 0) < 0 THEN '($' + CONVERT(VARCHAR(50), ABS(COALESCE(lb.current0, M.current0, 0)), 1) + ')' ELSE '$' + CONVERT(VARCHAR(50), COALESCE(lb.current0, M.current0, 0), 1) END AS LinkedBalance,
...
WHERE customgroupid = 24) THEN CASE esd.EFT WHEN 1 THEN 'Y' ELSE 'N' END ELSE ISNULL(ed3.line3, '') END AS [L3_Line3],
	...
	ISNULL(RIGHT(m.account, 4), '') AS L4_Line4,
...
ISNULL((SELECT TOP 1 w.AccountNumber
FROM Wallet w WITH (NOLOCK)
	 INNER JOIN WalletContact wc WITH (NOLOCK) ON w.ID = wc.WalletId
WHERE w.AccountType <> 'credit' AND w.DebtorId = sd.DebtorID
ORDER BY w.ID DESC), ISNULL(dbi.AccountNumber, '')) AS [BankInfo.AccountNumber],
...
ORDER BY w.ID DESC), ISNULL(dbi.BankPhone, '')) AS [BankInfo.Phone],
	...
	CASE WHEN COALESCE(lb.current0, M.current0, 0) < 0 THEN '($' + CONVERT(VARCHAR(50), ABS(COALESCE(lb.current0, M.current0, 0)), 1) + ')' ELSE '$' + CONVERT(VARCHAR(50), COALESCE(lb.current0, M.current0, 0), 1) END AS LinkedCurrentBalance,
...
--commented out until tested with letter vendor
	...
	FROM CTE_HeaderData AS M
		 LEFT JOIN CTE_ExtraData AS ED ON M.number = ED.number
		 INNER JOIN customer cu ON cu.customer = M.customer
		 LEFT OUTER JOIN restrictions r ON r.debtorid = M.debtorid
		 LEFT OUTER JOIN attorney a ON a.AttorneyId = M.AttorneyId
		 LEFT OUTER JOIN CourtCases cc ON cc.AccountID = M.number
		 LEFT OUTER JOIN Garnishment g ON M.number = g.AccountID
		 LEFT OUTER JOIN Courts co ON co.CourtID = cc.CourtID
		 LEFT OUTER JOIN Courts jco ON (
		 (
		 ...
		 LEFT OUTER JOIN extradata ed1 ON ed1.number = M.number AND
		 ed1.extracode = 'L1'
		 LEFT OUTER JOIN extradata ed2 ON ed2.number = M.number AND
		 ed2.extracode = 'L2'
		 LEFT OUTER JOIN extradata ed3 ON ed3.number = M.number AND
		 ed3.extracode = 'L3'
		 LEFT OUTER JOIN extradata ed4 ON ed4.number = M.number AND
		 ed4.extracode = 'L4'
		 LEFT OUTER JOIN StateRestrictions sr ON sr.abbreviation = M.State
		 INNER JOIN Desk de ON de.code = M.desk
		 INNER JOIN BranchCodes bc ON bc.code = de.Branch
		 LEFT OUTER JOIN Debtors sd ON sd.debtorid = M.subjdebtorid
		 LEFT OUTER JOIN Bankruptcy b ON b.debtorid = M.subjdebtorid
		 LEFT OUTER JOIN Deceased dc ON dc.debtorid = M.subjdebtorid
		 LEFT OUTER JOIN CCCS cs ON cs.debtorid = M.subjdebtorid
		 LEFT OUTER JOIN EarlyStageData esd ON esd.AccountID = M.number
		 LEFT OUTER JOIN PatientInfo pati ON pati.AccountID = M.number
		 LEFT OUTER JOIN DebtorAttorneys da ON da.debtorid = M.subjdebtorid
		 LEFT OUTER JOIN Users usend ON usend.ID = M.SenderID
		 LEFT OUTER JOIN Users ureq ON ureq.ID = M.RequesterID
		 LEFT OUTER JOIN debtorbankinfo dbi ON dbi.AcctID = sd.number AND
		 dbi.debtorid = sd.SEQ
		 LEFT OUTER JOIN LinkedBalances lb ON M.Link = lb.Link
		 LEFT OUTER JOIN Settlement sa ON M.number = sa.AccountID AND
		 M.SettlementID = sa.ID
		 LEFT OUTER JOIN (SELECT AcctID,
		 MIN(CONVERT(VARCHAR(20), Promises.entered, 101)) AS PromiseEntered
		 FROM dbo.Promises
		 WHERE Promises.Active = 1
		 GROUP BY AcctID) Promises ON Promises.AcctID = M.number
		 LEFT OUTER JOIN (SELECT Number,
		 MIN(CONVERT(VARCHAR(20), Pdc.entered, 101)) AS PDCEntered
		 FROM dbo.Pdc
		 WHERE Pdc.Active = 1
		 GROUP BY Number) Pdc ON Pdc.number = M.number
		 LEFT OUTER JOIN (SELECT Number,
		 ...
		 GROUP BY Number) DebtorCreditCards ON DebtorCreditCards.Number = M.number

		 LEFT OUTER JOIN dbo.ChargedOffBalanceDetailTotal ON M.Number = ChargedOffBalanceDetailTotal.AccountID
