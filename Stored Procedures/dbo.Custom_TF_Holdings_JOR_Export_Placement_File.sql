...
-- =============================================
CREATE PROCEDURE [dbo].[Custom_TF_Holdings_JOR_Export_Placement_File]
...
BEGIN
	...
	Zipcode, IsVIP,	IsCovid, Address_1, Address_2, City, State 

        FROM Custom_TF_Holdings_JOR_Import_Active_Accounts ctfhjoriaa WITH (NOLOCK)
        Where ctfhjoriaa.loan_series_number NOT IN 
				(SELECT account
		FROM master WITH (NOLOCK)
		WHERE (closed IS NULL OR status IN ('pif', 'sif', 'cnd', 'cad', 'lcp', 'rsk', 'aty', 'lit', 'fcd', 'dec', 'bky', 'b07', 'b11', 'b13'))
		AND (customer IN ('0003006')))
