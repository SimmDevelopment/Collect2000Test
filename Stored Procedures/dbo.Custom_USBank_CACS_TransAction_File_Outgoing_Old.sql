...
-- =============================================
CREATE PROCEDURE [dbo].[Custom_USBank_CACS_TransAction_File_Outgoing_Old] 
...
FROM master m WITH (NOLOCK) INNER JOIN debtors d WITH (NOLOCK) ON m.number = d.number
	INNER JOIN StatusHistory sh WITH (NOLOCK) ON m.number = sh.AccountID
WHERE 
m.customer IN (SELECT customerid FROM Fact f WITH (NOLOCK) WHERE CustomGroupID = @CustGroupID) --Production
