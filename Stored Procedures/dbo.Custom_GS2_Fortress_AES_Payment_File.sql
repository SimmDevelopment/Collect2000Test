...
-- =============================================
CREATE PROCEDURE [dbo].[Custom_GS2_Fortress_AES_Payment_File]
...
where customer in ('0001298', '0001360', '0001434', '0001435', '0001443', '0001524', '0001529', '0001674', '0001694','0001754','0001763','0001775','0001810','0001811','0001812','0001813','0001814','0001815','0001816','0001821','0002367') and invoiced = dbo.date(getdate()) and batchtype = 'pu'
...
select 
	...
	'10' as financialacttype,
	'10' as financialactsubtype,
	replace(convert(varchar(10), p.datepaid, 110), '-', '') as effectivedate,
	replace((p.paid1 + p.paid2 + OverPaidAmt), '.', '') as paymentamount,
	replace(convert(varchar(10), datepaid, 110), '-', '') as remitbatchdaterev,
	case when batchtype like '%R' then 'XX' else '' end as revreasonrejectcode,
	...
	--CASE WHEN p.checknbr IS NULL THEN '0001' WHEN p.checknbr = '' THEN '0001' ELSE p.checknbr END as loansequence,
	'PA' as regionind
from payhistory p with (nolock) inner join master m with (nolock) on p.number = m.number
where p.customer in ('0001298', '0001360', '0001434', '0001435', '0001443', '0001524', '0001529', '0001674', '0001694','0001754','0001763','0001775','0001810','0001811','0001812','0001813','0001814','0001815','0001816','0001821','0002367') and invoiced = dbo.date(getdate()) and batchtype = 'pu'
...
select 'TRAILER' as recordtype,
	...
	replace(sum(case when batchtype like '%r' then -(p.paid1 + p.paid2 + OverPaidAmt) else (p.paid1 + p.paid2 + OverPaidAmt) end), '.', '') as totalpayments,
	'' as filler,
	'PA' as regionind
from payhistory p with (nolock) inner join master m with (nolock) on p.number = m.number
where p.customer in ('0001298', '0001360', '0001434', '0001435', '0001443', '0001524', '0001529', '0001674', '0001694','0001754','0001763','0001775','0001810','0001811','0001812','0001813','0001814','0001815','0001816','0001821','0002367') and invoiced = dbo.date(getdate()) and batchtype = 'pu'
