...
EXEC Custom_Suntrust_BBT_Export_Financials  23376

*/


--ALTER  the procedure.
CREATE      PROCEDURE [dbo].[Custom_Suntrust_BBT_Export_Financials]
--@BeginDate datetime,
--@EndDate datetime,
...
CREATE TABLE #Suntrust_BBT_TempUploadFinancials
(
TransDate datetime,
TransTime VARCHAR(4),
Account VARCHAR(20),
...
INSERT INTO #Suntrust_BBT_TempUploadFinancials
(
TransDate,
TransTime,
Account,
...
SELECT REPLACE(CONVERT(char, GetDate(), 111), '/', ''),
...
--SELECT CASE WHEN ph.batchtype in ('PU','PA') THEN REPLACE(CONVERT(char, GetDate(), 111), '/', '') ELSE (SELECT datepaid FROM payhistory WHERE uid = ph.reverseofuid) END,
    ...
    --            WHEN ph.batchtype IN ('PUR','PAR') THEN Round([dbo].[Custom_CalculatePaymentTotalFee](ph.uid) * -1,2)
       ...
       InterestFlag = 'N',
       DirectedFlag = '',     
       transdesc = 'Fees - collection agency',
  --         CASE
		--WHEN ph.batchtype IN ('PU','PA') THEN 'Payment - SIM'
  --              WHEN ph.batchtype IN ('PUR','PAR') THEN 'Payment Reversal-NSF SIM '
	 --  END,
...
UPDATE #Suntrust_BBT_TempUploadFinancials
...
Select REPLACE(CONVERT(char, TransDate, 111), '/', '') as BatchDate, TransDate, 
       ...
       [dbo].[Custom_ConvertNumberToIBMSigned](CONVERT(varchar(5),((SELECT COUNT(Account)
        FROM #Suntrust_BBT_TempUploadFinancials))),5, 0) as TransCount,
       [dbo].[Custom_ConvertNumberToIBMSigned](CONVERT(varchar(10),((SELECT SUM(TransAmt)
        FROM #Suntrust_BBT_TempUploadFinancials))),10, 1) as GrossTransAmt,

       [dbo].[Custom_ConvertNumberToIBMSigned](CONVERT(varchar(10),((SELECT SUM(NetPmtAmt)
        FROM #Suntrust_BBT_TempUploadFinancials))),10, 1) as GrossNetAmt,
       ...
       CASE WHEN transcode = '1J' THEN '0000' ELSE CONVERT(varchar(4),ABS(CommPerValue)) END as CommPercValue,
       --[dbo].[Custom_ConvertNumberToIBMSigned](CONVERT(varchar(4),ABS(0)),4,0) as CommPercValue,
       ' ' as NotUsed
from #Suntrust_BBT_TempUploadFinancials
GO
