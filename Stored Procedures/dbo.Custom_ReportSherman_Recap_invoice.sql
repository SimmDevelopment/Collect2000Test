...
*/
CREATE  PROCEDURE [dbo].[Custom_ReportSherman_Recap_invoice]
...
BEGIN
	...
	Select	
			@paymentAgencyStr,
			Count(*),						--num payments
			Sum(isnull(dbo.Custom_CalculatePaymentTotalPaid(ph.uid),0) + isnull(ph.overpaidamt,0)) ,		
			Sum(isnull(dbo.Custom_CalculatePaymentTotalFee(ph.uid),0))			
		From PayHistory ph
		where ph.invoice in (select string from dbo.CustomStringToSet(@invoice,'|') )
	...
	Insert into @PaymentSummary([PaymentType],[NumPayments],[PaymentSum],[CommissionSum])
	Select	
			@misappliedPaymentStr,
			Count(*),						--num payments
			-(Sum(isnull(dbo.Custom_CalculatePaymentTotalPaid(ph.uid),0) + isnull(ph.overpaidamt,0))) ,		
			-(Sum(isnull(dbo.Custom_CalculatePaymentTotalFee(ph.uid),0)))			
		From PayHistory ph
		where ph.invoice in (select string from dbo.CustomStringToSet(@invoice,'|') )
		and ph.batchtype in ('PAR','PUR') and ph.iscorrection = 1
		Group by ph.batchtype
		...
		Select
			...
			isnull(Sum(isnull(dbo.Custom_CalculatePaymentTotalPaid(ph.uid),0) + isnull(ph.overpaidamt,0)),0) ,		
			isnull(Sum(isnull(dbo.Custom_CalculatePaymentTotalFee(ph.uid),0)),0)		
		From PayHistory ph
		where ph.invoice in (select string from dbo.CustomStringToSet(@invoice,'|') )
		and ph.batchtype in ('PUR','PAR') and ph.iscorrection = 0
		--Group by ph.batchtype
