...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[Propensio_Profile_EmailAdded]
...
BEGIN
    ...
    BEGIN TRY
        ...
        SET Email = @EmailAddress,
            contactmethod = 'Email' -- set preferred contact method to email
        WHERE DebtorID = @debtorID;
        --Write note
        INSERT INTO notes
        (
        ...
        (@masterNumber, @dateupdated, 'PAYWEB', '+++++', '+++++',
         'Email information has been updated from PAYWEB - Email Address Added - ' + @EmailAddress, 0);
        --Write history
        ...
        --remove all current entries that have the same email address.
        ...
        --Write history for email change
        INSERT INTO dbo.EmailHistory
        (
            AccountID,
            DebtorID,
        ...
        --remove old emails with same address
        DELETE FROM EmailDebtorsHistory
        WHERE DebtorId = @debtorID
              AND Email = @EmailAddress;
        --Insert new email
        ...
        --Write history for emaildebtorshistory change
        INSERT INTO dbo.EmailHistory
        (
            AccountID,
            DebtorID,
        ...
        VALUES
        ('[Propensio_Profile_EmailAdded]', @AccountId, @UserId, @Username, @IPAddress, @ContactId, @EmailAddress,
         @statusmessage);
        COMMIT TRANSACTION;
    END TRY
    BEGIN CATCH
