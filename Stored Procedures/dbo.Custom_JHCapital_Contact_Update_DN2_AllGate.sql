...
-- =============================================
Create PROCEDURE [dbo].[Custom_JHCapital_Contact_Update_DN2_AllGate] 
...
WHERE customer IN (SELECT customerid FROM fact WITH (NOLOCK) WHERE customgroupid  in (186,280)) AND NewNumber <> ''
AND closed IS NULL AND dbo.date(datechanged) BETWEEN dbo.date(@startDate) AND dbo.date(@endDate)
and id2 in ('AllGate')
