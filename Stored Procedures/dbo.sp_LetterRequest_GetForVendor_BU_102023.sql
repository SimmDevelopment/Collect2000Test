...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[sp_LetterRequest_GetForVendor_BU_102023]
       (
        @LetterID INT,
...
EXEC [dbo].[sp_LetterRequest_GetValidationExpirationForAllCustomers]
 ...
 ;     WITH  CTE_LetterAccountData
	...
	CASE WHEN M.AltRecipient = 1 THEN ISNULL(M.AltName, '') WHEN r.letterstoatty = 1 THEN ISNULL(da.Name, '') ELSE ISNULL(M.DebtorName, '') END AS Name,
...
WHEN m.customer IN ('0002468', '0002469') AND M.lettercode = 'PPWC1' AND M.JobName <> '' THEN M.JobName ELSE LTRIM(ISNULL(CASE sd.isParsed WHEN 1 THEN sd.firstName ELSE dbo.GetFirstName(sd.Name) END + ' ' + CASE sd.isParsed WHEN 1 THEN sd.lastName ELSE dbo.GetLastName(sd.Name) END, '')) END AS [FirstNameFirst],
	CASE WHEN M.AltRecipient = 1 THEN ISNULL(dbo.GetLastName(M.AltName), '') WHEN r.letterstoatty = 1 THEN ISNULL(da.Name, '') ELSE ISNULL(CASE M.isParsed WHEN 1 THEN M.lastName ELSE dbo.GetLastName(M.DebtorName) END, '') END AS LastName,
	CASE WHEN M.AltRecipient = 1 THEN ISNULL(dbo.GetFirstName(M.AltName), '') WHEN r.letterstoatty = 1 THEN ISNULL(da.Name, '') ELSE ISNULL(CASE M.isParsed WHEN 1 THEN M.firstName ELSE dbo.GetFirstName(M.DebtorName) END, '') END AS FirstName,
	ISNULL(M.OtherName, '') AS Other,
	CASE WHEN M.AltRecipient = 1 THEN ISNULL(M.AltStreet1, '') WHEN r.letterstoatty = 1 THEN ISNULL(da.Addr1, '') ELSE ISNULL(M.Street1, '') END AS Street1,
	CASE WHEN M.AltRecipient = 1 THEN ISNULL(M.AltStreet2, '') WHEN r.letterstoatty = 1 THEN ISNULL(da.Addr2, '') ELSE ISNULL(M.Street2, '') END AS Street2,
	CASE WHEN M.AltRecipient = 1 THEN ISNULL(M.AltCity, '') WHEN r.letterstoatty = 1 THEN ISNULL(da.City, '') ELSE ISNULL(M.City, '') END AS City,
	CASE WHEN M.AltRecipient = 1 THEN ISNULL(M.AltState, '') WHEN r.letterstoatty = 1 THEN ISNULL(da.State, '') ELSE ISNULL(M.State, '') END AS State,
	CASE WHEN M.AltRecipient = 1 THEN ISNULL(M.AltZipcode, '') WHEN r.letterstoatty = 1 THEN ISNULL(da.Zipcode, '') ELSE ISNULL(M.Zipcode, '') END AS Zipcode,
	CASE WHEN M.AltRecipient = 1 THEN CASE WHEN (LEN(ISNULL(M.AltCity, '')) > 0) THEN ISNULL(M.AltCity, '') + ', ' + ISNULL(M.AltState, '') + ' ' + ISNULL(M.AltZipcode, '') ELSE ISNULL(M.AltState, '') + ' ' + ISNULL(M.AltZipcode, '') END WHEN r.letterstoatty = 1 THEN CASE WHEN (LEN(ISNULL(da.City, '')) > 0) THEN ISNULL(da.City, '') + ', ' + ISNULL(da.State, '') + ' ' + ISNULL(da.Zipcode, '') ELSE ISNULL(da.State, '') + ' ' + ISNULL(da.Zipcode, '') END ELSE CASE WHEN (LEN(ISNULL(M.City, '')) > 0) THEN ISNULL(M.City, '') + ', ' + ISNULL(M.State, '') + ' ' + ISNULL(M.Zipcode, '') ELSE ISNULL(M.State, '') + ' ' + ISNULL(M.Zipcode, '') END END AS CSZ,
