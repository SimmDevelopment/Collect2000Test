...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[Dashboard_GetDepartmentPerformance]
   @ManagerID INT,
   @Date DATETIME = NULL,
   @UseGross BIT = 0
AS
BEGIN
    ...
    --jjh: added 0.999 so it would include the current day
    SELECT IDENTITY(INT, 1,1) AS 'Day', [Date]+0.999 as [Date]
    INTO #tmpLMDates
    FROM calendar
    ...
    --jjh: added last last month code  
    ...
    BEGIN
        ...
        (
            ...
            LEFT JOIN departments dpt ON dpt.ID = teams.DepartmentID 
            WHERE  ManagerID = @ManagerID AND YEAR(entered) = YEAR(DATEADD(month, -1, @Date))
        ...
        SELECT  COALESCE(SUM(CASE WHEN SUBSTRING(invoiceflags, 1, 1) = 1 THEN 
            ...
            LEFT JOIN teams ON teams.ID = desk.TeamID 
            LEFT JOIN departments dpt ON dpt.ID = teams.DepartmentID 
            WHERE  ManagerID = @ManagerID AND YEAR(entered) = YEAR(@lastlastMonth)
        ...
        (
            ...
            LEFT JOIN teams ON teams.ID = desk.TeamID 
            LEFT JOIN departments dpt ON dpt.ID = teams.DepartmentID
            WHERE  ManagerID = @ManagerID AND YEAR(entered) = YEAR(@Date)
            AND MONTH(entered) = MONTH(@Date)
      
            GROUP BY t.LM_Date, t.Day
         ) v1 ON v1.Day = l.Day
    END
    ELSE
    BEGIN
        ...
        (
            ...
            LEFT JOIN departments dpt ON dpt.ID = teams.DepartmentID
            WHERE  ManagerID = @ManagerID	 AND YEAR(entered) = YEAR(DATEADD(month, -1, @Date))
            AND MONTH(entered) = MONTH(DATEADD(month, -1, @Date))
            GROUP BY  t.[Day]
        ) v ON v.[Day] = l.[Day]
        LEFT JOIN
        (
