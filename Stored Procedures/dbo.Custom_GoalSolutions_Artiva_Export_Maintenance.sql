...
-- Description:	Export Maintenance File for Goal Solutions Placements by Date
-- Changes:
-- =============================================
CREATE PROCEDURE [dbo].[Custom_GoalSolutions_Artiva_Export_Maintenance]
	-- Add the parameters for the stored procedure here
	@startDate DATE,
	@endDate DATE	

--	exec Custom_GoalSolutions_Artiva_Export_Maintenance '20221212', '20221212'
...
--500 Record Placement Acknowledgments
	SELECT '500' AS RECID, m.account AS ACCTNUM, '' AS AGENCYID, 'N' AS ISRETURN, '' AS RETURNREASON, 'ACT' AS VENDORSTATUS, '' AS OBJECTREASON, '' AS DISPUTEREASON, 
	'' AS DISPUTEDATE, '' AS DISPUTETYPE, '' AS PAIDDATE, '' AS PAIDAMOUNT, '' AS OTHERREASON, '' AS COMPLAINTREASON, '' AS COMPLAINTDATE, '' AS COMPLAINTTYPE
--	(SELECT TOP 1 TheData from MiscExtra WITH (NOLOCK) WHERE m.number = number AND Title = 'acc.0.afacintrate') AS AFACINTRATE,
...
WHERE customer = '0002983' AND CAST(m.received AS DATE) BETWEEN CAST(@startDate AS DATE) AND CAST(@endDate AS DATE)
...
FROM master m WITH (NOLOCK) INNER JOIN Debtors d WITH (NOLOCK) ON m.number = d.Number AND d.Seq = 0
WHERE customer = '0002983' AND CAST(m.returned AS DATE) BETWEEN CAST(@startDate AS DATE) AND CAST(@endDate AS DATE)
...
WHERE customer = '0002983' AND m.status = 'VRD'
...
ISNULL(FORMAT(m.lastpaid, 'MMddyyyy'), '') AS ARACLPYDTE,  'N' AS ARACDISPUTE, '' AS ARACDISPRSNID, '' AS ARACDISPDTE, '' AS ZZACDSPPDDTE, '' AS ZZACDSPPDAMT, '' AS ZZACDSPOTHREAS
FROM master m WITH (NOLOCK) INNER JOIN Bankruptcy b WITH (NOLOCK) ON m.number = b.AccountID INNER JOIN Debtors d WITH (NOLOCK) ON b.DebtorID = d.DebtorID
WHERE customer = '0002983' AND m.status = 'VRB'
...
WHERE customer = '0002983' AND m.status = 'RSK' AND CAST(closed  AS DATE) BETWEEN CAST(@startDate AS DATE) AND CAST(@endDate AS DATE)
...
ISNULL(FORMAT(m.lastpaid, 'MMddyyyy'), '') AS ARACLPYDTE,  'N' AS ARACDISPUTE, '' AS ARACDISPRSNID, '' AS ARACDISPDTE, '' AS ZZACDSPPDDTE, '' AS ZZACDSPPDAMT, '' AS ZZACDSPOTHREAS
FROM master m WITH (NOLOCK) 
WHERE customer = '0002983' AND m.status = 'MIL' AND CAST(closed  AS DATE) BETWEEN CAST(@startDate AS DATE) AND CAST(@endDate AS DATE)
