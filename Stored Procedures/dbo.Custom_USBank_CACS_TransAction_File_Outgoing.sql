...
--				04/17/2020 BGM Added 3PHU note result to 425 records to pick up.
...
--				12/20/2022 KSE changed 8A to go back when in VOD status instead of PVR- Per Pam
...
--				09/15/2023 BGM Updated to catch previously statused as POO but then changed to other status prior to being reported back to US Bank.
--				09/15/2023 BGM Added text to extension requests for Accounts with claims as EXTENSION REQUEST??-??CLAIM??PRESENTED
...
--				01/24/2024 BGM Updated Language List Record 425
--				01/24/2024 BGM Added Email and SMS Text codes 3N and 3S
--				01/26/2024 BGM Added status CVD to Custom Data Segment - CEA, Record 400 Status Changes - Cease and Desist and Record 400 Status Changes - Validation of Debt VOD
--				01/26/2024 BGM Added status FVD to Record 400 Status Changes - Fraud and Record 400 Status Changes - Dispute account DPV Closed
--				01/29/2024 BGM Added 9T to compliment 9S for Lanuage Affirmation
-- =============================================
CREATE PROCEDURE [dbo].[Custom_USBank_CACS_TransAction_File_Outgoing] 
	-- Unused Add the parameters for the stored procedure here
	--@startDate DATETIME,
	--@endDate DATETIME
AS
BEGIN
...
SET @CustGroupID = 113 --Test	
	...
	--exec Custom_USBank_CACS_TransAction_File_Outgoing '20200508', '20200508'
...
--Record 425 Preferred Language Affirmation Updated Primary added 01/29/2024
SELECT DISTINCT id2 AS LocationCode, m.account AS Acctnumber, '425' AS TransCode, CONVERT(VARCHAR(8), GETDATE(), 112) AS TransDate,
'9T' AS AgyActCode, 
CONVERT(VARCHAR(8), n.created, 112) AS AgyActDate, REPLACE(CONVERT(VARCHAR(10), n.created, 8), ':', '') AS AgyActTime, 
'SIMM    ' AS ThirdPartyID,
'SYSTEM' AS AgyCollID, 
'Y' + ',' + FORMAT(n.created, 'yyyyMMdd')
  AS AgyHistoryTxt
FROM master m WITH (NOLOCK) INNER JOIN Debtors d WITH (NOLOCK) ON m.number = d.Number AND d.seq = 0
INNER JOIN notes n WITH (NOLOCK) on m.number = n.number
WHERE n.created BETWEEN @startDate AND @endDate 
...
--Record 425 Preferred Language Updated Codebtor added 02/08/2021
SELECT DISTINCT id2 AS LocationCode, m.account AS Acctnumber, '425' AS TransCode, CONVERT(VARCHAR(8), GETDATE(), 112) AS TransDate,
'9S' AS AgyActCode, 
CONVERT(VARCHAR(8), n.created, 112) AS AgyActDate, REPLACE(CONVERT(VARCHAR(10), n.created, 8), ':', '') AS AgyActTime, 
'SIMM    ' AS ThirdPartyID,
...
--Record 425 Preferred Language Affirmation Updated Codebtor added 01/29/2024
SELECT DISTINCT id2 AS LocationCode, m.account AS Acctnumber, '425' AS TransCode, CONVERT(VARCHAR(8), GETDATE(), 112) AS TransDate,
'9T' AS AgyActCode, 
CONVERT(VARCHAR(8), n.created, 112) AS AgyActDate, REPLACE(CONVERT(VARCHAR(10), n.created, 8), ':', '') AS AgyActTime, 
'SIMM    ' AS ThirdPartyID,
...
--Record 425 Preferred Language Updated Codebtor-2 added 03/04/2024
SELECT DISTINCT id2 AS LocationCode, m.account AS Acctnumber, '425' AS TransCode, CONVERT(VARCHAR(8), GETDATE(), 112) AS TransDate,
'9S' AS AgyActCode, 
CONVERT(VARCHAR(8), n.created, 112) AS AgyActDate, REPLACE(CONVERT(VARCHAR(10), n.created, 8), ':', '') AS AgyActTime, 
'SIMM    ' AS ThirdPartyID,
...
CASE d.language	
	...
	ELSE 'ZX'			 
END  AS AgyHistoryTxt
FROM master m WITH (NOLOCK) INNER JOIN Debtors d WITH (NOLOCK) ON m.number = d.Number AND d.seq = 2
INNER JOIN notes n WITH (NOLOCK) on m.number = n.number
WHERE n.created BETWEEN @startDate AND @endDate 
AND convert(varchar(300), n.comment) LIKE 'Debtor(3) Language was:%'
AND customer IN (SELECT customerid FROM Fact f WITH (NOLOCK) WHERE CustomGroupID = @CustGroupID)
and [action] = '+++++' AND user0 IN (SELECT LoginName FROM users WITH (NOLOCK) WHERE ACTIVE = 1)
AND result = '+++++'
AND (closed IS NULL OR closed BETWEEN @startDate AND DATEADD(dd, 1, @endDate))
...
--Record 425 Preferred Language Affirmation Updated Codebtor-1 added 03/04/2024
SELECT DISTINCT id2 AS LocationCode, m.account AS Acctnumber, '425' AS TransCode, CONVERT(VARCHAR(8), GETDATE(), 112) AS TransDate,
'9T' AS AgyActCode, 
CONVERT(VARCHAR(8), n.created, 112) AS AgyActDate, REPLACE(CONVERT(VARCHAR(10), n.created, 8), ':', '') AS AgyActTime, 
'SIMM    ' AS ThirdPartyID,
...
--Record 425 Phone Calls Outbound, 300 Comment Part1
SELECT DISTINCT id2 AS LocationCode, m.account AS Acctnumber, '425' AS TransCode, CONVERT(VARCHAR(8), GETDATE(), 112) AS TransDate,
...
--#region Letters, Email, Text Sent
...
--Record 425 Email Sent
SELECT DISTINCT id2 AS LocationCode, m.account AS Acctnumber, '425' AS TransCode, CONVERT(VARCHAR(8), GETDATE(), 112) AS TransDate,
'3N' AS AgyActCode, 
CONVERT(VARCHAR(8), n.created, 112) AS AgyActDate, REPLACE(CONVERT(VARCHAR(10), n.created, 8), ':', '') AS AgyActTime, 
'SIMM    ' AS ThirdPartyID,
'SYSTEM' AS AgyCollID, 
convert(VARCHAR(255), n.comment) + ' Contact: ' + d.name + ' on ' + format(n.created, 'MM/dd/yyyy') + ' at ' + format(n.created, 'HH:mm:ss')  AS AgyHistoryTxt
FROM master m WITH (NOLOCK) INNER JOIN Debtors d WITH (NOLOCK) ON m.number = d.Number AND d.seq = 0
INNER JOIN notes n WITH (NOLOCK) on m.number = n.number
WHERE n.created BETWEEN @startDate AND @endDate 
AND customer IN (SELECT customerid FROM Fact f WITH (NOLOCK) WHERE CustomGroupID = @CustGroupID)
and [action] in ('EMAIL') AND user0 IN ('Quanta')
AND result IN ('SEND')
AND (closed IS NULL OR closed BETWEEN @startDate AND DATEADD(dd, 1, @endDate))

UNION ALL

--Record 425 Text SMS Message Sent
SELECT DISTINCT id2 AS LocationCode, m.account AS Acctnumber, '425' AS TransCode, CONVERT(VARCHAR(8), GETDATE(), 112) AS TransDate,
'3S' AS AgyActCode, 
CONVERT(VARCHAR(8), n.created, 112) AS AgyActDate, REPLACE(CONVERT(VARCHAR(10), n.created, 8), ':', '') AS AgyActTime, 
'SIMM    ' AS ThirdPartyID,
...
WHERE n.created BETWEEN @startDate AND @endDate 
AND customer IN (SELECT customerid FROM Fact f WITH (NOLOCK) WHERE CustomGroupID = @CustGroupID)
and [action] in ('TXT1') AND user0 IN ('SBT')
AND result IN ('T-OK')
AND (closed IS NULL OR closed BETWEEN @startDate AND DATEADD(dd, 1, @endDate))
...
CASE WHEN m.status IN ('CAD', 'CVD', 'CND') AND m.number IN (SELECT number FROM notes WITH (NOLOCK) WHERE number = m.number AND result = 'VCD' AND created BETWEEN @startDate AND @endDate) THEN 'Verbal' WHEN status IN ('VCD', 'HCD') THEN 'Verbal' ELSE 'Written' END  + ', Requested Cease and Desist'  AS StatusText
FROM master m WITH (NOLOCK) INNER JOIN StatusHistory sh WITH (NOLOCK) ON m.number = sh.AccountID
WHERE m.customer IN (SELECT customerid FROM Fact f WITH (NOLOCK) WHERE CustomGroupID = @CustGroupID)
AND sh.DateChanged BETWEEN @startDate AND @endDate AND status IN ('CND', 'CAD', 'CVD', 'VCD', 'HCD')
...
WHERE m.customer IN (SELECT customerid FROM Fact f WITH (NOLOCK) WHERE CustomGroupID = @CustGroupID)
AND (CAST(sh.DateChanged AS DATE) BETWEEN @startDate AND @endDate AND sh.NewStatus IN ('POO')
or (CAST(sh.DateChanged AS DATE) BETWEEN @startdate AND @endDate AND sh.OldStatus IN ('POO')
AND CAST(sh.DateChanged AS DATE)  <= @startDate AND sh.NewStatus IN ('POO')))
...
'DISPUTE' AS StatusCode, 'Notified on ' + CONVERT(VARCHAR(8), sh.DateChanged, 112) + ', ' + CASE WHEN m.status IN ('DSP') THEN 'Debtor is disputing the balance of the account' END AS StatusText
FROM master m WITH (NOLOCK) INNER JOIN debtors d WITH (NOLOCK) ON m.number = d.number
	INNER JOIN StatusHistory sh WITH (NOLOCK) ON m.number = sh.AccountID
WHERE m.customer IN (SELECT customerid FROM Fact f WITH (NOLOCK) WHERE CustomGroupID = @CustGroupID) --Production
...
'EXTREQ1' AS StatusCode, 'As of ' + CONVERT(VARCHAR(8), GETDATE(), 112) + ', ' + CASE WHEN m.status IN ('CLM', 'POC') THEN 'EXTENSION REQUEST??-??CLAIM??PRESENTED' ELSE CONVERT(VARCHAR(150), n.comment) END AS StatusText
...
SELECT DISTINCT id2 AS LocationCode, m.account AS Acctnumber, '400' AS TransCode, CONVERT(VARCHAR(8), sh.DateChanged, 112) AS TransDate,
'FRAUD' AS StatusCode, 'Notified on ' + CONVERT(VARCHAR(8), sh.DateChanged, 112) + ', ' + CASE WHEN m.status in ('FRD', 'FVD') THEN 'Debtor is stating fraudulent charges made on account or account opened without their knowledge' END AS StatusText
FROM master m WITH (NOLOCK) INNER JOIN debtors d WITH (NOLOCK) ON m.number = d.number
	INNER JOIN StatusHistory sh WITH (NOLOCK) ON m.number = sh.AccountID
WHERE m.customer IN (SELECT customerid FROM Fact f WITH (NOLOCK) WHERE CustomGroupID = @CustGroupID)
AND sh.DateChanged BETWEEN @startDate AND @endDate AND status IN ('FRD', 'FVD')
AND m.closed BETWEEN @startDate AND @endDate
...
SELECT DISTINCT id2 AS LocationCode, m.account AS Acctnumber, '400' AS TransCode, CONVERT(VARCHAR(8), sh.DateChanged, 112) AS TransDate,
'VOD' AS StatusCode, 'Notified on ' + CONVERT(VARCHAR(8), sh.DateChanged, 112) + ', ' + CASE WHEN m.status IN ('CVD', 'FVD', 'DSP') THEN 'Debtor requesting validation of debt for duration of account life' ELSE '' END AS StatusText
FROM master m WITH (NOLOCK) INNER JOIN debtors d WITH (NOLOCK) ON m.number = d.number
	INNER JOIN StatusHistory sh WITH (NOLOCK) ON m.number = sh.AccountID
WHERE m.customer IN (SELECT customerid FROM Fact f WITH (NOLOCK) WHERE CustomGroupID = @CustGroupID)
AND sh.DateChanged BETWEEN @startDate AND @endDate AND status IN ('CVD', 'FVD', 'DSP')
