...
*/
CREATE    PROCEDURE [dbo].[Custom_Reactivate]
            @number as int
AS
BEGIN
            ...
            --gets info for note          
            SELECT @received = received, @original = original,@original1 = original1,@original2 = original2,@original3 = original3,
            ...
            UPDATE MASTER SET
                        ...
                        ,ORIGINAL10 = 0
            FROM MASTER M WITH(NOLOCK)
            JOIN Custom_Reactivations CS WITH(NOLOCK) ON CS.Account = M.ACCOUNT
            WHERE M.NUMBER = @number
            ...
            SET @Action = '+++++'
            SET @Result = '+++++'
            SET @Comment = 'Acct reassigned, old assign date '+cast(@received as varchar(30))+', old original = ' + cast(@original as varchar(15))+', old current = '+cast(@current0 as varchar(15))
