...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[Propensio_Profile_EmailUpdated]
...
BEGIN
    ...
    BEGIN TRY
        ...
        (@masterNumber, @dateupdated, 'PAYWEB', '+++++', '+++++',
         'Email information has been updated from PAYWEB - Email Updated - Old: ' + @OldEmailAddress + ' to New: '
         + @EmailAddress, 0);
        ...
        VALUES
        ('[Propensio_Profile_EmailUpdated]', @AccountId, @UserId, @Username, @IPAddress, @ContactId, @EmailAddress,
         @OldEmailAddress, @statusmessage);
        COMMIT TRANSACTION;
    END TRY
    BEGIN CATCH
        IF XACT_STATE() <> 0
