...
set @startDate='20130730'
set @endDate='20130730'
--set @endDate='12/20/2006 12:15:00'
set @customer='0001257|'
exec Custom_Pay_Pal_Upload_nopay @startDate=@startDate,@endDate=@endDate,@customer=@customer
*/
CREATE                        PROCEDURE [dbo].[Custom_Pay_Pal_Upload_NoPay]
	 @startDate datetime,
	 @endDate datetime,
...
LEFT OUTER JOIN master m with (nolock) ON t.number = m.number
LEFT OUTER JOIN statushistory s WITH(NOLOCK) ON s.accountid= m.number
WHERE m.status in ('VRB', 'BKO', 'BKY', 'B07', 'B11', 'B13', 'VCD', 'pdc', 'pcc', 'ppa', 'nsf', 'dcc', 'dbd', 'npc', 'hot', 'mhd', 'nhd') 
AND m.customer IN (select string from dbo.CustomStringToSet(@customer, '|'))
)
