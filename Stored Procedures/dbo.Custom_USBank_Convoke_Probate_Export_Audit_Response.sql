...
-- =============================================
CREATE PROCEDURE [dbo].[Custom_USBank_Convoke_Probate_Export_Audit_Response]
...
or m.number IN (SELECT number FROM notes WITH (NOLOCK) WHERE number = m.number AND action = 'USBC' AND result = 'AUDCR' AND created BETWEEN @startDate AND @endDate))
END
GO
