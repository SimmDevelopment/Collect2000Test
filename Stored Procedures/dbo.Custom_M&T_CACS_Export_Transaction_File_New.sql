...
-- =============================================
CREATE PROCEDURE [dbo].[Custom_M&T_CACS_Export_Transaction_File_New] 
...
FROM    Custom_MT_Temp_Placement cmtp WITH ( NOLOCK ) INNER JOIN Custom_MT_Temp_LegalData cmtld ON cmtp.AcctNumber = cmtld.AcctNumber
WHERE TransactionCode IN ('210', '211', '110', '111', '213')
...
SELECT  cmtca.AgencyId, cmtca.LocationCode, cmtca.AcctNum, cmtca.TransCode, cmtca.TransDate, cmtca.ContactId, cmtca.ContactRecType,
        ...
        cmte.EmailAddress, cmtld.PrincipalAmt, cmtld.OthDelinqAmt, cmtld.IntOwingAtCo, cmtld.ChargeOffDate
FROM    Custom_MT_Temp_ContactAcct cmtca WITH ( NOLOCK )
		...
		LEFT OUTER JOIN Custom_MT_Temp_LegalData cmtld WITH ( NOLOCK ) ON cmtca.AcctNum = cmtld.AcctNumber 
WHERE LeadContactInd = 'Y'
AND (cmtca.TransCode IN ('210', '211', '110', '111', '213') or cmtbc.TransCode IN ('210', '211', '110', '111', '213') or cmta.TransCode IN ('210', '211', '110', '111', '213')
or cmte.TransCode IN ('210', '211', '110', '111', '213')or cmtld.TransCode IN ('210', '211', '110', '111', '213'))
...
WHERE cmta.TransCode IN ('210', '211', '110', '111', '213') AND PrefferredAddressInd <> 'Y'
...
WHERE cmte.TransCode IN ('210', '211', '110', '111', '213') AND PreferredEmailAddressInd <> 'Y'
...
FROM    Custom_MT_Temp_Phones cmtp WITH ( NOLOCK ) 
WHERE cmtp.TransCode IN ('210', '211', '110', '111', '213')
