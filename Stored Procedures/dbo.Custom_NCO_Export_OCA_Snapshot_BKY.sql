...
-- =============================================
create PROCEDURE [dbo].[Custom_NCO_Export_OCA_Snapshot_BKY] 
...
BEGIN
	...
	select convert(varchar(8), getdate(), 112) as asofdate, m.number as account, d1.ssn, SUBSTRING(account, CHARINDEX('-', account, 1) + 1, 3) as loanseqnum, 
...
where m.customer in (select customerid from fact with (Nolock) where customgroupid = 174) and m.closed between @startDate and @endDate and m.status in ('bky', 'b07', 'b11', 'b13', 'dec')
