...
-- =============================================
CREATE PROCEDURE [dbo].[Custom_TF_Holdings_Export_Placement_File]
...
BEGIN
	...
	Zipcode, CreditLimit,	PayFrequency, IsCovid, Address_1, Address_2, City, State
        ...
        FROM Custom_TF_Holdings_Import_Active_Accounts ctfhiaa WITH (NOLOCK)
        Where ctfhiaa.loan_series_number NOT IN 
				(SELECT account
		FROM master WITH (NOLOCK)
		WHERE (closed IS NULL OR status IN ('pif', 'sif', 'cnd', 'cad', 'lcp', 'rsk', 'aty', 'lit', 'fcd', 'dec', 'bky', 'b07', 'b11', 'b13'))
		AND (customer IN ('0002998')))
