...
SET ANSI_NULLS ON
GO
/*dbo.sp_CbrForcedDisputesToPending*/
CREATE      PROCEDURE [dbo].[sp_CbrForcedDisputesToPending]
	@ErrNum	INT OUTPUT
-- Name:	sp_CbrForcedDisputesToPending
-- Function:	This procedure will insert CbrForced disputes to CbrPendingFile table
...
AS
	...
	select	f.AccountId, @runDate, 0, 0, '', '93', 'XB',
	case
		when
	...
	where f.Type = 'DISPUTE'

	IF (@@Error!=0)
	BEGIN
    	SET @ErrNum = @@Error
    	RAISERROR  ('20000',16,1,'Error encountered in sp_CbrForcedDisputesToPending.')
    	RETURN(1)
	END	
