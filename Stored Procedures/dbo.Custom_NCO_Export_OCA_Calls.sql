...
-- =============================================
create PROCEDURE [dbo].[Custom_NCO_Export_OCA_Calls]
...
where m.customer in (select customerid from fact with (Nolock) where customgroupid = 174) and dbo.date(created) between @startDate and @endDate and (action like 't%' or action in ('dt', 'mnual', '3P'))
