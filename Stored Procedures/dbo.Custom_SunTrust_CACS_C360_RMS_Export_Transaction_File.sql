...
-- BGM - 05/20/2021 Changed Placement record to return transaction code 209 when it is 210
-- BGM - 10/27/2021 Added record 112 212 for Debt Validation
-- =============================================
CREATE PROCEDURE [dbo].[Custom_SunTrust_CACS_C360_RMS_Export_Transaction_File] 
...
SELECT  AgencyId, LocationCode, AcctNumber, CASE WHEN TransactionCode = '210' THEN '209' ELSE TransactionCode END AS TransactionCode, 
        ...
        ChargeOffFee, ChargeOffDate, BankNumber, BranchName, Lobcode
        
FROM    Custom_SunTrust_C360_RMS_Temp_Placement cubtp WITH ( NOLOCK ) 
...
--Debt Validation
SELECT DISTINCT cstdv.ThirdPartyID,cstdv.LocationCode,cstdv.AcctNumber,cstdv.TransCode,cstdv.LastUpdate,cstdv.AsOfDate,cstdv.AsOfDateDescription,cstdv.BalanceOnAOD,cstdv.IntChgSinceAOD
			   ,cstdv.FeesChgSinceAOD,cstdv.OthDebtsSinceAOD,cstdv.PaysSinceAOD,cstdv.OtherCreditSinceAOD,cstdv.CurrBalanceOnLastUpdate
FROM    Custom_Suntrust_Temp_Debt_Validation cstdv WITH ( NOLOCK ) 
WHERE cstdv.TransCode IN ('212', '112')
