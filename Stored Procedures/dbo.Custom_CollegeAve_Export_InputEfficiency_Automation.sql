...
-- =============================================
CREATE PROCEDURE [dbo].[Custom_CollegeAve_Export_InputEfficiency_Automation] 
...
BEGIN
	...
	select @Automation_StartDate = (select param1 from custom_automate_run where job_name = '(A) CA Export 13')
...
WHERE CAST(ProductionDate AS DATE) = CAST(DATEADD(dd, -1, @Automation_StartDate) AS DATE)
