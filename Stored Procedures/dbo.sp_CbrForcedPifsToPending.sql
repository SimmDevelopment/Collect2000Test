...
SET ANSI_NULLS ON
GO
/*dbo.sp_CbrForcedPifsToPending*/
CREATE      PROCEDURE [dbo].[sp_CbrForcedPifsToPending]
	@ErrNum	INT OUTPUT
-- Name:	sp_CbrForcedPifsToPending
-- Function:	This procedure will insert CbrForced Pifs to CbrPendingFile table
...
AS
	...
	select	f.AccountId, @runDate, m.Current0, m.Current1, 'PIF', '62', '',
	case
		when
	...
	BEGIN
    	SET @ErrNum = @@Error
    	RAISERROR  ('20000',16,1,'Error encountered in sp_CbrForcedPifsToPending.')
    	RETURN(1)
	END	
