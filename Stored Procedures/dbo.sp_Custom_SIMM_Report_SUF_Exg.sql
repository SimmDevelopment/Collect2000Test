...
CREATE PROCEDURE [dbo].[sp_Custom_SIMM_Report_SUF_Exg]
	@CustGroupId int
AS
BEGIN
...
-- *************** get the CCA info here ************************************************
	...
	-- Added by KAR on 02/18/2010 Need to capture KPI Records
	--Count of notes entered for each account in the user selected customer(s) within the user selected date range 
		--where the notes.result code is set to result.contacted = '1' (contacted) in the result table
	--Count of notes entered for each account in the user selected customer(s) within the user selected date range 
		--where the notes.action code is set to action.WasAttempt = '1' (attempted) in the action table
	--Count of notes entered for each account in the user selected customer(s) within the user selected date range for customers included 
		--where the notes.result code is set to result.worked = '1' (worked) in the result table
	SELECT COUNT(*) AS NumAccts
	From master m with (nolock)--From Debtors d with (nolock)
	where m.customer in (select CustomerId from Fact with (nolock) where CustomGroupId=@custGroupId)
	and m.Qlevel NOT IN('999')
	...
	SELECT 
		...
		(Select top 1 TheData from MiscExtra with (nolock) where number = m.number and Title = 'ACCTID')				as [AcctId],
		m.account			as [AccountNumber],
		ISNULL((SELECT COUNT(DISTINCT n.created) FROM Notes n with (nolock)
		       where n.number = m.number AND 
			   result IN(SELECT code FROM result WITH(NOLOCK) WHERE contacted = 1)),0) as [RightPartyContact], -- Count of notes entered for each account in the user selected customer(s) within the user selected date range 
																												-- where the notes.result code is set to result.contacted = '1' (contacted) in the result table
		ISNULL((SELECT COUNT(DISTINCT n.created) FROM Notes n with (nolock)
		       where n.number = m.number AND 
			   action IN(SELECT code FROM action WITH(NOLOCK) WHERE WasAttempt = 1)),0) as [CallAttempts], --Count of notes entered for each account in the user selected customer(s) within the user selected date range 
																											--where the notes.action code is set to action.WasAttempt = '1' (attempted) in the action table		
		ISNULL((SELECT COUNT(DISTINCT n.created) FROM Notes n with (nolock)
		       where n.number = m.number AND 
			   result IN(SELECT code FROM result WITH(NOLOCK) WHERE Worked = 1)),0) as [Connects], -- Count of notes entered for each account in the user selected customer(s) within the user selected date range 
																										--for customers included where the notes.result code is set to result.worked = '1' (worked) in the result table
		'0' as [Letters], -- zero fill	
		'0.00' as [LastSIFPercentOffered], --	0.##	zero fill	
		'' as [DateofLastSIFOffered]	-- 0.##	zero fill	
		From master m with (nolock)--From Debtors d with (nolock)
		where m.customer in (select CustomerId from Fact with (nolock) where CustomGroupId=@custGroupId)
		and m.Qlevel NOT IN('999')
		--m.number IN(SELECT number from notes n with (Nolock)
        --            WHERE n.number = m.number AND 
		--			((result IN(SELECT code FROM result WITH(NOLOCK) WHERE contacted = 1)) OR
		--			 (result IN(SELECT code FROM result WITH(NOLOCK) WHERE worked = 1)) OR
		--			 (action IN(SELECT code FROM action WITH(NOLOCK) WHERE WasAttempt = 1))))
