...
CREATE PROCEDURE [dbo].[Dashboard_GetCustomerGroupPerformance](@CustomGroupID INT, @Date DATETIME, @MonthsBack INT)
AS
BEGIN
   ...
   JOIN Fact F ON M.Customer = F.CustomerID
   JOIN CustomCustGroups CG ON F.CustomGroupID = CG.ID
   WHERE CG.ID = @CustomGroupID
     AND C.[status] = 'active' 
   ...
   JOIN CUSTOMER C ON P.Customer = C.Customer
   JOIN FACT F ON P.Customer = F.CustomerID
   JOIN CUSTOMCUSTGROUPS CG ON F.CustomGroupID = CG.ID
   WHERE CG.ID = @CustomGroupID
     AND C.[status] = 'active'
