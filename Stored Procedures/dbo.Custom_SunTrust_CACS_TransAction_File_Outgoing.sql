...
-- =============================================
Create PROCEDURE [dbo].[Custom_SunTrust_CACS_TransAction_File_Outgoing] 
...
SELECT id2 AS LocationCode, m.account AS Acctnumber, '425' AS TransCode, CONVERT(VARCHAR(8), GETDATE(), 112) AS TransDate,
	CASE WHEN user0 = 'SIMM' AND action = 'INT' AND result = 'SRCH' THEN '8X'
		 ...
		 WHEN user0 = 'TLO' AND action = 'SEND' AND result = 'SEND' AND comment LIKE '%bankruptcy%' THEN '8W'
	END AS AgyActCode, 
...
AND customer IN (SELECT customerid FROM Fact f WITH (NOLOCK) WHERE CustomGroupID = 107)
AND user0 IN ('SIMM', 'TLO', 'FUSION', 'DECDSVC', 'WEBRECON', 'TLO')
...
REPLACE(CONVERT(VARCHAR(18), p.totalpaid), '.', '') + '0000' AS AmountPaid, REPLACE(CONVERT(VARCHAR(18), p.CollectorFee), '.', '') + '0000' AS CommissionAmt, 'SIMM    ' AS ThirdPartyID, '646' AS InputSRCcode
...
AND m.customer IN (SELECT customerid FROM Fact f WITH (NOLOCK) WHERE CustomGroupID = 107)
...
REPLACE(CONVERT(VARCHAR(14), p.totalpaid), '.', '') + '0000' AS AmountPaid, REPLACE(CONVERT(VARCHAR(18), p.CollectorFee), '.', '') + '0000' AS CommissionAmt, 'SIMM    ' AS ThirdPartyID, '646' AS InputSRCcode
FROM master m WITH (NOLOCK) INNER JOIN payhistory p WITH (NOLOCK) ON m.number = p.number
...
WHERE m.customer IN (SELECT customerid FROM Fact f WITH (NOLOCK) WHERE CustomGroupID = 107)
...
WHERE m.customer IN (SELECT customerid FROM Fact f WITH (NOLOCK) WHERE CustomGroupID = 107)
AND n.created BETWEEN @startDate AND @endDate AND user0 = 'STB' AND action = 'RCL' AND result = 'RSN'
...
'A' AS FunctionCode, 'SIMM      ' AS AgencyId, REPLACE(CONVERT(VARCHAR(14), m.current0), '.', '') + '0000' AS BalAtSettle, (select REPLACE(CONVERT(VARCHAR(14), SUM(amount)), '.', '') + '0000' FROM pdc WITH (NOLOCK) WHERE number = p.number AND (p.DateCreated BETWEEN @startDate AND @endDate OR p.DateUpdated BETWEEN @startDate AND @endDate)) AS SettlementOffered, 
...
CASE WHEN DATEDIFF(MM, (select TOP 1 CONVERT(VARCHAR(8), deposit, 112) FROM pdc WITH (NOLOCK) WHERE number = p.number AND (p.DateCreated BETWEEN @startDate AND @endDate OR p.DateUpdated BETWEEN @startDate AND @endDate) ORDER BY deposit ASC), (select TOP 1 CONVERT(VARCHAR(8), deposit, 112) FROM pdc WITH (NOLOCK) WHERE number = p.number AND (p.DateCreated BETWEEN @startDate AND @endDate OR p.DateUpdated BETWEEN @startDate AND @endDate) ORDER BY deposit DESC)) > 18 THEN 'Y ' + 'STB Manager' ELSE 'N' END AS StlmtNeedsSTBApprov, 
