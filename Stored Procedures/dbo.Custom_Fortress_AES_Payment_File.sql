...
-- =============================================
CREATE PROCEDURE [dbo].[Custom_Fortress_AES_Payment_File]
...
where customer in ('0001034') and invoiced = dbo.date(getdate()) and batchtype = 'pu'
...
select 
	...
	(SELECT TOP 1 SUBSTRING(thedata, 1, 3) FROM miscextra WITH (NOLOCK) WHERE number = p.number AND title = 'rec.0.servicerseq') AS loansequence,
...
where p.customer in ('0001034') and invoiced = dbo.date(getdate()) and batchtype = 'pu'
