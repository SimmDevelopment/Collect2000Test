...
SET ANSI_NULLS ON
GO
CREATE  PROC [dbo].[Custom_Process_TLO_Deceased]
	@RequestID INT
AS
-- Name:		Custom_Process_TLO_Deceased
-- Function:		This procedure will process returned TLO Deceased info
-- Creation:		07/10/2012 Brian Meehan
-- Modified:			
--		BGM 10/13/2011 - Added code to trigger ASTS export in AIM to pick up Deceased hits.	
--		BGM 07/07/2015 - Added code to not close FMH accounts and only mark them as FDC status.
--		BGM 07/14/2015 - Corrected code to allow Probate accounts to remain open when Deceased information is returned.
-- Change History:	
	...
	IF (SELECT COUNT(*) FROM [dbo].[Services_TLO_Deceased] AS [Services_TLO_Deceased]
		WHERE [Services_TLO_Deceased].[RequestID] = @RequestID) > 0 
		BEGIN
	...
	BEGIN
		...
		SELECT TOP 1 @number, @DebtorID, [Services_TLO_Deceased].[SSN], [Services_TLO_Deceased].[TLOFirstName], [Services_TLO_Deceased].[TLOLastName],
			[Services_TLO_Deceased].[TLOState], [Services_TLO_Deceased].[TLOZip], 
			[Services_TLO_Deceased].[DateofBirth],
			[Services_TLO_Deceased].[DateofDeath]
		FROM [dbo].[Services_TLO_Deceased] AS [Services_TLO_Deceased]
		WHERE [Services_TLO_Deceased].[RequestID] = @RequestID
		IF (@@ERROR != 0) GOTO ErrHandler
	END
...
ErrHandler:
	RAISERROR('Error encountered in Custom_TLO_Process_Deceased for account id %d.  Custom_TLO_Process_Deceased failed.', 11, 1, @number)
	RETURN
GO
