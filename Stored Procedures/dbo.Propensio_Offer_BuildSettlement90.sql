...
CREATE procedure [dbo].[Propensio_Offer_BuildSettlement90]
...
AS
	...
	SET @Payment1Amount = ROUND(@Balance*.9, 2, 0)
	...
	INSERT INTO [dbo].[SettlementOffers]
     ...
     VALUES
           (@SettlementId,
			...
			'A payment of 90% of your current balance in ONE PAYMENT of $' + CONVERT(varchar, @Payment1Amount) + ' that must be received in this office on or before ' + CONVERT(varchar, @MaxDate1, 101),
			@Payment1Amount,
			0,
			@Tier,
           'STLMNT90', -- This is arbitrary. Can be used for reporting/metrics
           GETDATE(),
		   GETDATE())
	...
	SET @Payment1Amount = ROUND((@Balance*.92)/2, 2, 0)
	...
	INSERT INTO [dbo].[SettlementOffers]
     ...
     VALUES
           (@SettlementId,
			...
			'A payment of 92% of your current balance in TWO PAYMENTS of $' + CONVERT(varchar, @Payment1Amount) + ' that must be received in this office. The first payment on or before ' + CONVERT(varchar,@MaxDate1, 101) + ' with the second and final payment due by ' + CONVERT(varchar, @MaxDate2,101) ,
			 @Payment1Amount * 2,
			0,
			@Tier,
           'STLMNT92', -- This is arbitrary. Can be used for reporting/metrics
           GETDATE(),
		   GETDATE())
