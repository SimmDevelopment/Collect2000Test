...
/*spCopyTableSchemav4*/
CREATE  PROCEDURE [dbo].[spCopyTableSchemav4] @srctable NVARCHAR(256), @desttable NVARCHAR(256)
AS 
...
SELECT [syscolumns].[colid] AS [colid], [syscolumns].[name] AS [name], 
	[systypes].[name] AS [type], [systypes].[status] AS [status], 
	[syscolumns].[length] AS [length], [syscolumns].[isnullable] AS [isnullable], 
	[syscomments].[text] AS [default] 
From [syscolumns] 
INNER JOIN [systypes] 
ON [syscolumns].[xtype] = [systypes].[xtype] 
LEFT OUTER JOIN [sysconstraints] 
ON [sysconstraints].[id] = [syscolumns].[id] 
...
DECLARE @cschema CURSOR
SET @cschema = CURSOR LOCAL FORWARD_ONLY READ_ONLY STATIC FOR 
