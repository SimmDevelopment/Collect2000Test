...
-- =============================================
CREATE PROCEDURE [dbo].[Custom_Citizens_FP_Export_NonWorkable_Maintenance] 
...
WHERE (acct_num NOT IN (SELECT account FROM master WITH (NOLOCK) WHERE customer = '0002226') OR acct_num IN (SELECT account FROM master WITH (NOLOCK) WHERE customer = '0002226' AND desk = '300' AND status = 'rcl'))
