...
*/
CREATE     PROCEDURE [dbo].[Custom_Simms_JCAP_Closure_JM_824]
...
AND m.status not in ('SIF')
	...
	FROM master m with (nolock)
--	JOIN PayHistory p with (nolock) ON p.number=m.number
	WHERE (m.number in (select m.number FROM master m with (nolock) WHERE m.closed between dbo.F_START_OF_WEEK(dateadd(dd, -1, getdate()), 6) AND dbo.F_END_OF_WEEK(dateadd(dd, -1, getdate()), 6) AND m.customer = '0001020' AND m.returned is null AND m.status not in ('SIF'))
	or m.number in (select m.number FROM master m with(nolock) where m.customer = '0001020' AND m.returned is null AND m.closed < (getdate()-15) AND m.status in ('SIF')))
