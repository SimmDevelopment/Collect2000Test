...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[SP_DeskChange0]     /**Use This SP to make simple Desk Changesdesk **/ 
   @number int, 
   @Qlevel varchar (3),                  /**the Distribution Job Object Uses SP_DeskChange2 **/     
...
BEGIN TRANSACTION 
   /*******************************************Get Local Variables*********************************************************************************************************************/ 
   SELECT  @Balance = Current0, @OldDesk = desk, @Cust = customer 
   FROM master  
   WHERE number = @number 
   SELECT @TheYear = currentyear, @TheMonth = currentmonth 
   FROM ControlFile 
   /*******************************************Update Master.desk*********************************************************************************************************************/ 
   UPDATE master SET desk = @newdesk,Qlevel = @Qlevel, QDate= @qdate 
   WHERE number = @number 
