...
--EXEC Custom_NCO_NCT_Tert_Export '23223', '20130512', '20130514'

CREATE PROCEDURE [dbo].[Custom_NCO_NCT_Tert_Export]
...
IF (@keepers = 1)
	BEGIN
    --Get record 30 payments
		SELECT '30' AS recordcode, m.id1 AS fileno, m.account AS forw_file, m.id2 AS masco_file, CASE WHEN m.customer = '0001283' THEN 'SIM3' WHEN m.customer = '0001404' THEN 'SIM9' END AS firm_id, CASE WHEN m.customer = '0001283' THEN 'ASLMSA' WHEN m.customer = '0001404' THEN 'ASLMSA' END AS forw_id,
		CASE WHEN status IN ('sif', 'PIF') THEN '3' ELSE '1' END AS ret_code, CASE WHEN batchtype = 'pur' THEN (SELECT TOP 1 CONVERT(VARCHAR(10), datepaid, 112) FROM payhistory WITH (NOLOCK) WHERE uid = p.reverseofuid) ELSE CONVERT(VARCHAR(10), p.datepaid, 112) END AS pay_date,
		CASE WHEN batchtype = 'pur' THEN -(p.paid1 + p.paid2) ELSE (p.paid1 + p.paid2) END AS gross_pay, '' AS net_client, '' AS check_amt, '' AS cost_ret,
		'' AS fees, '' AS agent_fees, '' AS forw_cut, '' AS cost_rec, '' AS bpj, uid AS ta_no, '' AS rmit_no, '' AS line1_3, '' AS line1_5, '' AS line1_6,
		'' AS line2_1, '' AS line2_2, '' AS line2_5, '' line2_6, '' AS line2_7, '' AS line2_8, '' AS [desc]
		FROM payhistory p WITH (NOLOCK) INNER JOIN master m WITH (NOLOCK) ON p.number = m.number
		WHERE invoice in (select string from dbo.CustomStringToSet(@invoice, '|')) AND batchtype LIKE 'pu%'

			--Get record 31 Address changes
		SELECT '31' AS recordcode, m.id1 AS fileno, m.account AS forw_file, m.id2 AS masco_file, CASE WHEN m.customer = '0001283' THEN 'SIM3' WHEN m.customer = '0001404' THEN 'SIM9' END AS firm_id, CASE WHEN m.customer = '0001283' THEN 'ASLMSA' WHEN m.customer = '0001404' THEN 'ASLMSA' END AS forw_id,
		...
		SELECT '33' AS recordcode
		, m.id1 AS fileno
		, m.account AS forw_file
		, m.id2 AS masco_file
		, CASE WHEN m.customer = '0001283' THEN 'SIM3' WHEN m.customer = '0001404' THEN 'SIM9' END AS firm_id
		, CASE WHEN m.customer = '0001283' THEN 'ASLMSA' WHEN m.customer = '0001404' THEN 'ASLMSA' END AS forw_id
		, d1.lastname AS DEBT2_LNAME
		, d1.firstName AS DEBT2_FNAME
		, d1.Street1 AS DEBT2_STREET1
		, d1.Street2 AS DEBT2_STREET2
		, d1.city AS DEBT2_CITY
		, d1.State AS DEBT2_STATE
		, d1.Zipcode AS DEBT2_ZIP
		, d1.HomePhone AS DEBT2_PHONE
		, d1.SSN AS DEBT2_SSN
		,  CONVERT(VARCHAR(10), d1.dob, 112) AS DEBT2_DOB
		, d1.DLNum AS DEBT2_DL
		, d2.lastname AS DEBT3_LNAME
		, d2.firstName AS DEBT3_FNAME
		, d2.Street1 AS DEBT3_STREET1
		, d2.Street2 AS DEBT3_STREET2
		, d2.city AS DEBT3_CITY
		, d2.State AS DEBT3_STATE
		, d2.Zipcode AS DEBT3_ZIP
		, d2.HomePhone AS DEBT3_PHONE
		, d2.SSN AS DEBT3_SSN
		, CONVERT(VARCHAR(10), d2.dob, 112) AS DEBT3_DOB
		, d2.DLNum AS DEBT3_DL
		from master m WITH (NOLOCK) LEFT OUTER JOIN debtors d1 WITH (NOLOCK) ON m.number = d1.number AND d1.seq = 1 
		LEFT OUTER JOIN debtors d2 WITH (NOLOCK) ON m.number = d2.number AND d2.seq = 2 
		LEFT OUTER JOIN dbo.AddressHistory ah1 WITH (NOLOCK) ON d1.DebtorID = ah1.DebtorID
		LEFT OUTER JOIN dbo.AddressHistory ah2 WITH (NOLOCK) ON d1.DebtorID = ah2.DebtorID
		where m.customer in (select string from dbo.CustomStringToSet(@customer, '|'))
		AND (dbo.date(ah1.DateChanged) BETWEEN @startDate AND @endDate OR dbo.date(ah2.DateChanged) BETWEEN @startDate AND @endDate)
		...
		SELECT '39' AS recordcode, m.id1 AS fileno, m.account AS forw_file, m.id2 AS masco_file, CASE WHEN m.customer = '0001283' THEN 'SIM3' WHEN m.customer = '0001404' THEN 'SIM9' END AS firm_id, CASE WHEN m.customer = '0001283' THEN 'ASLMSA' WHEN m.customer = '0001404' THEN 'ASLMSA' END AS forw_id,
		...
		SELECT DISTINCT '39' AS recordcode, m.id1 AS fileno, m.account AS forw_file, m.id2 AS masco_file, CASE WHEN m.customer = '0001283' THEN 'SIM3' WHEN m.customer = '0001404' THEN 'SIM9' END AS firm_id, CASE WHEN m.customer = '0001283' THEN 'ASLMSA' WHEN m.customer = '0001404' THEN 'ASLMSA' END AS forw_id,
			CONVERT(VARCHAR(10), p.entered, 112) AS pdate,
			'*CC:A103' AS pcode,
			'Payment Plan' AS pcmt
		...
		WHERE m.status IN ('SKP', 'AEX', 'BKN', 'DBD', 'DCC', 'DPV', 'LCP', 'RFP', 'BKY', 'B07', 'B11', 'B13', 'CCR', 'DEC', 'ATY', 'PIF', 'RCL', 'SIF', 'FRD', 'DSP', 'CAD', 'CND', 'CCS') AND dbo.date(sh.DateChanged) BETWEEN @startDate AND @endDate AND returned IS null
		AND m.customer in (select string from dbo.CustomStringToSet(@customer, '|'))
	END
ELSE
	BEGIN
    --Get record 30 payments
		...
		d.HomePhone AS DEBT3_PHONE, d.SSN AS DEBT3_SSN,  CONVERT(VARCHAR(10), d.dob, 112) AS DEBT3_DOB, d.DLNum AS DEBT3_DL
